	...
	$.ajax({
		...
		success:function (data) {
			if(data.error) {  // If there is an error, show the error messages
				...
				$("#button_holder").html("&nbsp;<br><span class='white_text'>A password reset link has been sent to your email.  Please check your inbox and follow the instructions.</span><div><button class='btn btn-lg btn-primary btn-block' onClick='document.location.href=\"https://starlinkcms.com\"' style='width:210px'>Login</button></div>");
				emptyReminderBuffer(data.success.text);
				setTimeout(function() {
					//document.location.href = "https://starlinkcms.com/";
				}, 5500);
			}
		}
	});
