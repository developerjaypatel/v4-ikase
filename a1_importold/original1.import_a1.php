...
try {
	...
	try {
		...
		foreach($sleeps as $sleep) {
			...
			$stmt = $dbConn->prepare($kill_command);
			$stmt->execute();
		}
		$stmt = null; $dbConn = null;
	...
	if (isset($_GET["id"])) {
		$sql = "DELETE FROM  `cse_case`
		WHERE cpointer = '" . $_GET["id"] . "'";
	...
	foreach($cases as $case_key=>$case){
		...
		echo "Processing -> " . $case_key. " == <a href='import_a1.php?customer_id=" . $customer_id . "&id=" .  $case->case_number . "'>" .  $case->case_number . "</a><br />\r\n";
		$time = microtime();
		$time = explode(' ', $time);
		$time = $time[1] + $time[0];
		...
		//now the kase
		$sql = "INSERT INTO `cse_case` (case_uuid, cpointer, case_number, case_date, case_type, venue, case_status, submittedOn, supervising_attorney, attorney, worker, customer_id) 
VALUES ('" . $case_uuid . "', '" . $case->case_number . "', '" . $case_number . "', '" . date("Y-m-d", strtotime($case->DATEENTER)) . "', '" . $case->CASETYPE . "', 'LAO', '" . addslashes($case->CASESTAT) . "', '" . date("Y-m-d", strtotime($case->DATEENTER)) . "', '" . $case->ATTY_RESP . "', '" . $case->ATTY_HAND . "', '" . $case->SEC_HAND . "', " . $customer_id . ")";
		...
		foreach($injuries as $injury_index=>$injury) {		
			...
			//if (!$blnCarrierOneAdded) {
				...
				if ($injury->I_NAME!="") {
					...
					$sql = "INSERT INTO `cse_case_corporation` (`case_corporation_uuid`, `case_uuid`, `corporation_uuid`, `injury_uuid`, `attribute`, `last_updated_date`, `last_update_user`, `customer_id`)
					...
					if ($injury->I_CLAIMNO!="") {
						...
						if (count($arrAdhocSet)>0) {
							//inserts
							$sql = "INSERT INTO `cse_corporation_adhoc` (`adhoc_uuid`, `case_uuid`, `corporation_uuid`, `adhoc`, `adhoc_value`, `customer_id`) VALUES ";
							$arrValues = array();
