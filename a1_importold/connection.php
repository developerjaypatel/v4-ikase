...
function make_bitly_url($url) {
	...
	$format = 'json';
	//create the URL
	$bitly = 'https://api-ssl.bitly.com/v4/shorten?access_token=32a1753df6a6a6ac67764772d9e3aabae50ae4f6&longUrl=' . urlencode($url);
...
function create_zip($files = array(),$destination = '',$overwrite = false) {
	...
	if(is_array($files)) {
		...
		foreach($files as $file) {
			//replace the url
			$file = str_replace("https://www.ikase.org", "..", $file);
