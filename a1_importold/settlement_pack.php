<?php
//$app->get('/settlement/:id', authorize('user'),	'getSettlement');
$app->get('/settlement/:injury_id', authorize('user'), 'getSettlements');
$app->get('/settlementfirst/:injury_id', authorize('user'), 'getFirstSettlement');
$app->get('/settlementsheet/:injury_id', authorize('user'), 'getSettlementSheet');
$app->get('/settlementsheetid/:id', authorize('user'), 'getSettlementSheetById');

//posts
//$app->post('/settlement/delete', authorize('user'), 'deleteSettlement');
$app->post('/settlement/add', authorize('user'), 'addSettlement');
$app->post('/settlement/update', authorize('user'), 'updateSettlement');
$app->post('/settlements/bydoctor/:doctors', authorize('user'), 'settlementsByDoctor');

$app->post('/settlementsheet/add', authorize('user'), 'addSettlementSheet');
$app->post('/settlementsheet/freeze', authorize('user'), 'freezeSettlementSheet');
$app->post('/settlementsheet/update', authorize('user'), 'updateSettlementSheet');
...
function getSettlements($injury_id) {
	...
	IFNULL(`cs`.`amount_of_fee`, 0) `amount_of_fee`, 
	IFNULL(`cs`.`c_and_r`, '') `c_and_r`, 
	IFNULL(`cs`.`stip`, '') `stip`, 
...
function getSettlementSheet($injury_id, $settlement_id = "", $blnReturn = false) {
	...
	try {
		...
		/*if (is_object($settlementsheet) && $settlement_id=="") { 
			//get rid of anything else
			//selected
			$settlementsheet_uuid = $settlementsheet->settlementsheet_uuid;
			$injury_uuid = $settlementsheet->injury_uuid;
...
function addSettlement() {
	...
	foreach($_POST as $fieldname=>$value) {
		...
		if ($fieldname=="settlement_id" || $fieldname=="table_id" || $fieldname=="nono") {
			continue;
		}
