...
$app->get('/bodyparts/box/:case_id/:injury_id', authorize('user'), 'getBodypartsBox');

$app->post('/bodyparts/add', authorize('user'), 'setBodyparts');
$app->post('/bodyparts/updatestatus', authorize('user'), 'updateStatus');
$app->post('/bodyparts/addtext', authorize('user'), 'addTextBox');

function getBodyparts($case_id, $injury_id) {
	session_write_close();
	//LEFT OUTER JOIN cse_injury_bodypartsbox cbox ON (cbox.injury_id = :injury_id AND `cbox`.`case_id` = :case_id) , cbox.*, cbox.bodyparts bodypartsbox 
    $sql = "SELECT DISTINCT bp.*, 
			cib.injury_bodyparts_id, cib.attribute bodyparts_number, cib.`status` `bodyparts_status`,
			ccase.case_id, ccase.case_uuid, bp.bodyparts_id id, cbox.bodyparts
			...
			LEFT OUTER JOIN ikase_alvandi.cse_injury_bodypartsbox cbox ON cbox.case_id=ccase.case_id and cbox.injury_id=ci.injury_id and cbox.customer_id = cci.customer_id
...
function getBodypartsBox($case_id, $injury_id) {
	session_write_close();
	
    $sql = "SELECT * FROM ikase_alvandi.cse_injury_bodypartsbox Where case_id = :case_id and injury_id = :injury_id;
			AND customer_id = '1195' AND deleted = 'N'";
	try {
		$db = getConnection();
		$stmt = $db->prepare($sql);
		$stmt->bindParam("case_id", $case_id);
		$stmt->bindParam("injury_id", $injury_id);
		$stmt->execute();
		$bodypartsbox = $stmt->fetchObject();
		$db = null;
		echo json_encode($bodypartsbox);
...
function addTextBox() {
	...
	$text_box = passed_var("text_box", "post");
	$injury_id = passed_var("injury_id", "post");
	...
	$count = 0;
	$sql = "SELECT * FROM ikase_alvandi.cse_injury_bodypartsbox Where case_id = :case_id and injury_id = :injury_id and customer_id = :customer_id AND deleted = 'N'";
	try {
		$db = getConnection();
		$stmt = $db->prepare($sql);
		$stmt->bindParam("case_id", $case_id);
	...
	if ($count>0){
	$sql = "update ikase_alvandi.cse_injury_bodypartsbox set bodyparts = :text_box Where case_id = :case_id and injury_id = :injury_id and customer_id = :customer_id AND deleted = 'N'";
			
	try {
		$db = getConnection();
		$stmt = $db->prepare($sql);
		$stmt->bindParam("text_box", $text_box);
		$stmt->bindParam("injury_id", $injury_id);
		$stmt->bindParam("case_id", $case_id);
		$stmt->bindParam("customer_id", $_SESSION['user_customer_id']);
		$stmt->execute();
		...
		echo json_encode(array("success"=>true, "text_box"=>$text_box, "injury_id"=>$injury_id, "case_id"=>$case_id)); 
	} catch(PDOException $e) {
		$error = array("error"=> array("text"=>$e->getMessage()));
        	echo json_encode($error);
	}
	} else {
	$sql = "INSERT INTO ikase_alvandi.cse_injury_bodypartsbox(`injury_id`, `case_id`, `bodyparts`, `customer_id`)
			VALUES(:injury_id, :case_id, :text_box, :customer_id)";
	try {
		$db = getConnection();
		$stmt = $db->prepare($sql);
		$stmt->bindParam("text_box", $text_box);
