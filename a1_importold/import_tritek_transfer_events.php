...
try {
	...
	$sql = "DELETE FROM `ikase_" . $data_source . "`.`cse_case_event` 
	WHERE 1 
	...
	$sql = "DELETE FROM `ikase_" . $data_source . "`.`cse_event` 
	WHERE 1
	AND customer_id = " . $customer_id . "
	";
	//AND event_uuid NOT IN (SELECT DISTINCT event_uuid FROM ikase_reino.cse_event_track)
	...
	IF (amflag='PM', DATE_ADD(`event_dateandtime`, INTERVAL 12 HOUR), `event_dateandtime`) `event_dateandtime`, 
	`event_end_time`, 
