...
try {
	...
	$sql = "SELECT gcase.* 
	FROM `" . $data_source . "`.`client` gcase
	LEFT OUTER JOIN `" . $data_source . "`.`" . $data_source . "_case` ggc
	ON gcase.cpointer = ggc.cpointer
	WHERE 1
	AND ggc.case_id IS NULL
	#AND gcase.cpointer = '611'
	#ORDER BY cpointer ASC
	LIMIT 0, 1";
	//echo $sql . "\r\n<br>";
    ...
    ref.comments referral_comments
		...
		foreach($injuries as $key=>$injury){
			...
			//now the kase
			$open_date = $injury->opendate;
			if ($open_date!="") {
				$open_date = date("Y-m-d", strtotime($injury->opendate));
			} else {
	...
	VALUES ('" . $case_uuid . "', '" . $injury->cpointer . "', '" . $injury->fileno . "', '" . addslashes($injury->casename) . "', '" .$open_date . "', '" . addslashes($injury->casetype) . "', 'LAO', '" . addslashes($injury->casestat) . "', '" . addslashes($injury->subcat1) . "', '" . addslashes($injury->subcat2) . "', '" . $open_date . "', '" . $injury->attycode . "', '" . $injury->workcode . "', " . $customer_id . ")";
			//echo $sql . "\r\n\r\n<br><br>"; 
			$stmt = $db->prepare($sql); 		
			$stmt->execute();
			//die();
			...
			VALUES('" . $injury_uuid . "', " . ($injury_number+1) . ", '" . addslashes($injury->caseno) . "', '', '" . addslashes($injury->occup) . "','" . $injury_accidate . "','" . addslashes($injury->body) . "','" . addslashes($injury->ctdates) . "','" . addslashes($injury->accidesc) . "','" . addslashes($full_address) . "', '" . addslashes($injury->acciadd1) . "','" . addslashes($injury->acciadd2) . "', '" . addslashes($injury->accicity) . "', '" . addslashes($injury->accist) . "', '" . $injury->accizip . "', " . $customer_id . ", 'N')";
			//echo $sql . "\r\n\r\n<br><br>"; 
			$stmt = $db->prepare($sql); 
			...
			$arrSet[] = addslashes(substr($injury->con_fax, 0, 255));
			...
			if ($injury->emp_firm!="") {
				...
				if (!is_object($employer)) {
					//insert the parent record first
					$sql = "INSERT INTO `" . $data_source . "`.`" . $data_source . "_corporation` (`corporation_uuid`, `customer_id`, `full_name`, `company_name`, `type`, `full_address`, `street`, `city`, `state`, `zip`, `suite`, `phone`, `email`, `salutation`, `last_updated_date`, `last_update_user`, `deleted`, `parent_corporation_uuid`, `copying_instructions`) 
		...
		echo " => QUERY completed in " . $total_time . "<br /><br />";
			...
			//negotiations
			$sql_neg = "SELECT `neg`.* 
			FROM `" . $data_source . "`.`neg`
			INNER JOIN `" . $data_source . "`.`client` cli
			ON `neg`.negpoint = cli.negpoint
			WHERE 1
			AND cli.cpointer = '" . $injury->cpointer . "'
			ORDER BY `date`";
			$stmt = $db->prepare($sql_neg);
			//echo $sql_neg . "\r\n\r\n<br><br>";
			//die();
			$stmt->execute();
			...
			foreach ($negs as $neg) {
				...
				$sql = "INSERT INTO `" . $data_source . "`.`" . $data_source . "_negotiation` 
				(`negotiation_uuid`, `negotiation_date`, `negotiator`, `firm`, `worker`, `negotiation_type`, `amount`, `comments`, `customer_id`)
				VALUES ('" . $table_uuid . "', '" . date("Y-m-d", strtotime($neg->date)) . " 00:00:00', '" . addslashes($neg->negotiator) . "', '" . addslashes($neg->firm) . "', '" . addslashes($neg->worker) . "', '" . $negotiation_type . "', '" . $neg->amount  . "', '" . addslashes($neg->comments) . "', '" . $customer_id . "')";
				$stmt = $db->prepare($sql); 
				//echo $sql . "\r\n\r\n<br><br>"; 
				$stmt->execute();
				...
				//now we have to attach the opposing to the case 
				$sql_neg = "INSERT INTO `" . $data_source . "`.`" . $data_source . "_case_negotiation` (`case_negotiation_uuid`, `case_uuid`, `negotiation_uuid`, `attribute`, `last_updated_date`, `last_update_user`, `customer_id`)
				VALUES ('" . $case_table_uuid  ."', '" . $case_uuid . "', '" . $table_uuid . "', 'main', '" . $last_updated_date . "', 'system', '" . $customer_id . "')";
			...
			foreach ($wages as $wage) {
				...
				$table_uuid = uniqid("NG", false);
				$lost_date = $wage->lostdate;
				if ($lost_date!="") {
					$lost_date = date("Y-m-d", strtotime($wage->lostdate));
				}  else {
				...
				VALUES ('" . $table_uuid . "', '" . $lost_date . "', '" . $lost_date . "', '" . addslashes($neg->lostdesc) . "', '" . $wage->lostamt . "', '" . $customer_id . "')";
				$stmt = $db->prepare($sql_wage); 
				//echo $sql_wage . "\r\n\r\n<br><br>"; 
				$stmt->execute();
			...
			foreach ($medicals as $medical) {
				$table_uuid = uniqid("DR", false);
				//$parent_table_uuid = uniqid("PD", false);
				$parent_table_uuid = $medical->medpnt;
				if ($medical->medname!=""){
					$arrSet = array();
					...
					AND deleted = 'N'
					AND corporation_uuid = '" . $medical->medpnt . "'";
					...
					AND `medicals`.medpnt = '" . $medical->medpnt . "'
...
(`medicalbilling_uuid`, `corporation_uuid`, `user_uuid`, `billed`, `paid`, `adjusted`, `balance`, `customer_id`) ";
					$sql .= "SELECT DISTINCT 
					CONCAT('" . $medical->medpnt . "', '_', @curRow := @curRow + 1) AS `medicalbilling_uuid`,
					'" . $table_uuid . "', 'system', medbill.amount, medbill.payment, medbill.adjustment,  
					...
					AND med.mpointer = '" . $injury->cpointer . "'
		...
		*/
		...
		if ($completed_count == $case_count) {
			...
			$sql = " CREATE TABLE IF NOT EXISTS `" . $data_source . "`.`badvenues` (
			  `badvenues_id` int(11) NOT NULL AUTO_INCREMENT,
			  `cpointer` varchar(15) DEFAULT '',
			  `venue_uuid` varchar(45) DEFAULT NULL,
			  `venue_abbr` varchar(45) DEFAULT '',
			  `judge` varchar(255) DEFAULT '',
			  `deleted` varchar(45) DEFAULT 'N',
			  PRIMARY KEY (`badvenues_id`)
			) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
			
			$sql .= "
			TRUNCATE `" . $data_source . "`.`badvenues`;
			INSERT INTO `" . $data_source . "`.`badvenues` (cpointer, judge, venue_uuid, venue_abbr)
			SELECT DISTINCT cli.cpointer, IFNULL(ccrt.courtjudge, '') courtjudge, ven.venue_uuid, REPLACE(courts.courtkey, 'WCAB-', '') venue_abbr
			FROM `" . $data_source . "`.`client` cli
			
			INNER JOIN `" . $data_source . "`.`ccourts` ccrt ON cli.ctpnt = ccrt.courtpoint
			INNER JOIN `" . $data_source . "`.`courts` courts ON ccrt.courtpnt = courts.courtpnt
			
			INNER JOIN `ikase`.`cse_venue` ven
			ON REPLACE(courts.courtkey, 'WCAB-', '') = ven.venue_abbr
			
			WHERE courts.courtkey IS NOT NULL";
			
			$sql .= "
			CREATE TABLE IF NOT EXISTS " . $data_source . ".`badtasks` (
			  `badtasks_id` int(11) NOT NULL AUTO_INCREMENT,
			  `cpointer` varchar(15) DEFAULT '',
			  `deleted` enum('Y','N') DEFAULT 'N',
			  PRIMARY KEY (`badtasks_id`),
			  KEY `cpointer` (`cpointer`),
			  KEY `deleted` (`deleted`)
			) ENGINE=InnoDB DEFAULT CHARSET=utf8;
			
			TRUNCATE `" . $data_source . "`.`badtasks`;
			
			INSERT INTO " . $data_source . ".badtasks (cpointer) 
			SELECT DISTINCT evpointer FROM " . $data_source . ".todo;";
			
			$sql .= "
			CREATE TABLE IF NOT EXISTS " . $data_source . ".`badexams` (
			  `badexams_id` int(11) NOT NULL AUTO_INCREMENT,
			  `case_uuid` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
			  `cpointer` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
			  `processed` enum('Y','N') CHARACTER SET utf8 DEFAULT 'N',
			  PRIMARY KEY (`badexams_id`),
			  KEY `processed` (`processed`),
			  KEY `case_uuid` (`case_uuid`)
			) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
			
			TRUNCATE `" . $data_source . "`.`badexams`;
			
			INSERT INTO " . $data_source . ".badexams (cpointer, case_uuid)
			SELECT DISTINCT mpointer, ccase.case_uuid 
			FROM " . $data_source . ".medicals
			INNER JOIN " . $data_source . "." . $data_source . "_case ccase
			ON medicals.mpointer = ccase.cpointer;
			";
			
			$stmt = $db->prepare($sql);
			$stmt->execute();
			
			$sql = "CREATE TABLE IF NOT EXISTS " . $data_source . ".`baddeductions` (
			  `baddeductions_id` int(11) NOT NULL AUTO_INCREMENT,
			  `case_uuid` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
			  `cpointer` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
			  `processed` enum('Y','N') CHARACTER SET utf8 DEFAULT 'N',
			  PRIMARY KEY (`baddeductions_id`),
			  KEY `processed` (`processed`),
			  KEY `case_uuid` (`case_uuid`)
			) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
			
			TRUNCATE `" . $data_source . "`.`baddeductions`;
			
			INSERT INTO " . $data_source . ".baddeductions (cpointer, case_uuid)
			SELECT DISTINCT cpointer, ccase.case_uuid 
			FROM " . $data_source . ".specials
			INNER JOIN " . $data_source . "." . $data_source . "_case ccase
			ON specials.specpnt = ccase.cpointer";
			
			$stmt = $db->prepare($sql);
			$stmt->execute();
			
			$sql = "CREATE TABLE IF NOT EXISTS " . $data_source . ".`badsubro` (
						  `badsubro_id` int(11) NOT NULL AUTO_INCREMENT,
						  `case_uuid` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
						  `cpointer` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
						  `processed` enum('Y','N') CHARACTER SET utf8 DEFAULT 'N',
						  PRIMARY KEY (`badsubro_id`),
						  KEY `processed` (`processed`),
						  KEY `case_uuid` (`case_uuid`)
						) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
						
						TRUNCATE `" . $data_source . "`.`badsubro`;
						
						INSERT INTO " . $data_source . ".badsubro (cpointer, case_uuid)
			...
			WHERE ins.insprepcnt > 0";
			$stmt = $db->prepare($sql);
			$stmt->execute();
			
			$sql = "CREATE TABLE IF NOT EXISTS " . $data_source . ".`badmeds` (
						  `badmeds_id` int(11) NOT NULL AUTO_INCREMENT,
						  `case_uuid` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
						  `cpointer` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
						  `processed` enum('Y','N') CHARACTER SET utf8 DEFAULT 'N',
						  PRIMARY KEY (`badmeds_id`),
						  KEY `processed` (`processed`),
						  KEY `case_uuid` (`case_uuid`)
						) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
						
						TRUNCATE `" . $data_source . "`.`badmeds`;
						
						INSERT INTO " . $data_source . ".badmeds (cpointer, case_uuid)
			SELECT DISTINCT ccase.cpointer, ccase.case_uuid
			FROM " . $data_source . ".medicals med
			INNER JOIN " . $data_source . "." . $data_source . "_case ccase
			ON med.mpointer = ccase.cpointer";
			$stmt = $db->prepare($sql);
			$stmt->execute();
			
			$sql = "CREATE TABLE IF NOT EXISTS " . $data_source . ".`badfees` (
						  `badfees_id` int(11) NOT NULL AUTO_INCREMENT,
						  `case_uuid` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
						  `cpointer` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
						  `processed` enum('Y','N') CHARACTER SET utf8 DEFAULT 'N',
						  PRIMARY KEY (`badfees_id`),
						  KEY `processed` (`processed`),
						  KEY `case_uuid` (`case_uuid`)
						) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
						
						TRUNCATE `" . $data_source . "`.`badfees`;
						
						INSERT INTO " . $data_source . ".badfees (cpointer, case_uuid)
			SELECT DISTINCT ccase.cpointer, ccase.case_uuid
			FROM " . $data_source . ".compfees
			INNER JOIN " . $data_source . "." . $data_source . "_case ccase
			ON compfees.feepointer = ccase.cpointer";
			$stmt = $db->prepare($sql);
			$stmt->execute();
			
			$sql = "CREATE TABLE IF NOT EXISTS `" . $data_source . "`.`badcases` (
						  `badcases_id` int(11) NOT NULL AUTO_INCREMENT,
						  `case_uuid` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
						  `cpointer` varchar(15) COLLATE utf8_unicode_ci DEFAULT '',
						  `processed` enum('Y','N') CHARACTER SET utf8 DEFAULT 'N',
						  PRIMARY KEY (`badcases_id`),
						  KEY `processed` (`processed`),
						  KEY `case_uuid` (`case_uuid`)
						) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
			...
			INSERT INTO " . $data_source . ".badcases (cpointer, case_uuid)
			SELECT gcase.cpointer, gcase.case_uuid 
			FROM " . $data_source . "." . $data_source . "_case gcase
			LEFT OUTER JOIN ikase_" . $data_source . ".cse_case ccase
			ON gcase.cpointer = ccase.cpointer
			WHERE ccase.cpointer IS NULL;";
			
			$stmt = $db->prepare($sql);
			$stmt->execute();
			
			$sql = "ALTER TABLE `ikase_" . $data_source . "`.`cse_settlement` 
			ADD COLUMN IF NOT EXISTS `legacy_info` TEXT NULL AFTER `attorney`;
			
			UPDATE `" . $data_source . "`.compsetl
			SET submitted = ''
			WHERE submitted =  '  /  /';
