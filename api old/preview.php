...
if (!$path) {
	...
	if (strpos($path, ".pdf") !== false || strpos($path, ".png") !== false || strpos($path, ".wma") !== false || strpos($path, ".mp3") !== false || strpos($path, ".jpg") !== false) {
		if (strpos($path, ".pdf") !== false) {
			$iframe = '<iframe id="letter_frame" src="https://v2.ikase.org/' . $path . '" width="100%" height="800px"></iframe>';
			echo $iframe;
		} else {
		...
		if (strpos($path, ".pdf") !== false || strpos($path, ".doc") !== false) {
			$iframe = '<iframe id="letter_frame" src="https://docs.google.com/gview?url=https://v2.ikase.org/' . $path . '&embedded=true" width="100%" height="800px"></iframe>';
		} else {
			$iframe = '<iframe id="letter_frame" src="https://v2.ikase.org/' . $path . '" width="100%" height="800px"></iframe>';
		}
		echo $iframe;
...
window.history.replaceState("", "Preview", "https://v2.ikase.org/");
</script>