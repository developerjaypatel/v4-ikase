<?php 
$app->get('/webmail', authorize('user'), 'webEmailInbox');
$app->get('/limmail', authorize('user'), 'limEmailInbox');
$app->get('/gogetmymail', authorize('user'), 'goGetMail');
$app->get('/processmail', authorize('user'), 'processlimEmailInbox');
$app->get('/pingmail', authorize('user'), 'pinglimEmailInbox');
$app->get('/obtainmail/:customer_id/:user_id', authorize('user'), 'webEmailInbox');
$app->get('/getmail', authorize('user'), 'getEmailInbox');
$app->get('/syncmail', authorize('user'), 'syncMail');

$app->get('/webmail/read/:id', authorize('user'), 'readEmail');
$app->get('/webmail/test', authorize('user'), 'testEmail');

$app->get('/webmail/verify/:direction', authorize('user'), 'verifyEmail');

$app->get('/gmail/token', authorize('user'), 'gmailToken');
$app->post('/gmail/settoken', 'gmailSetToken');
$app->get('/gmail/refreshtoken', authorize('user'), 'refreshToken');
$app->post('/gmail/cleartoken', 'gmailClearToken');
$app->post('/gmail/dontuse', 'emailAccountDisactivate');
$app->post('/gmail/activate', authorize('user'), 'emailAccountActivate');
...
$app->post('/gmail/assign', authorize('user'), 'assignGmail');
...
function gmailSetToken() {
	...
	/*
	try {
		$sql = "DELETE FROM ikase.cse_gmail WHERE user_id = :user_id";
		$db = getConnection();
		$stmt = $db->prepare($sql);
	...
	*/
	...
	if (!is_object($user)) {
		$sql = "INSERT INTO ikase.cse_gmail (user_id, token, origin)
		VALUES(:user_id, :token, :origin)";
		$db = getConnection();
		$stmt = $db->prepare($sql);
		$stmt->bindParam("user_id", $user_id);
		$stmt->bindParam("token", $gtoken);
	...
	try {
		...
		foreach($arrAttachments as $attachment) {
			...
			if ($strpos === false) {
				...
				array_push($arrAttachedFiles, "<a href='https://v2.ikase.org/uploads/" . $attachment . "' target='_blank' onmouseover='showImportedPreview(this, \"uploads/" . $attachment . "\", \"\", \"\", " . $attach_customer_id . ", \"activity_\")' onmouseout='hidePreview()'>" . $attach_file . "</a>");
			...
			if ($attach_customer_id==$_SESSION["user_customer_id"] && $attach_user_id==$_SESSION["user_plain_id"]) {
				...
				*/
				...
				$uploadDir = "\\uploads\\" . $attach_customer_id . "\\" . $case_id;
				if (!is_dir($_SERVER['DOCUMENT_ROOT'] . $uploadDir)) {
					mkdir($_SERVER['DOCUMENT_ROOT'] . $uploadDir, 0755, true);
				}
				$uploadDir = "\\uploads\\" . $attach_customer_id . "\\" . $case_id . "\\medium";
				if (!is_dir($_SERVER['DOCUMENT_ROOT'] . $uploadDir)) {
					mkdir($_SERVER['DOCUMENT_ROOT'] . $uploadDir, 0755, true);
				}
				$uploadDir = "\\uploads\\" . $attach_customer_id . "\\" . $case_id . "\\thumbnail";
				if (!is_dir($_SERVER['DOCUMENT_ROOT'] . $uploadDir)) {
					mkdir($_SERVER['DOCUMENT_ROOT'] . $uploadDir, 0755, true);
				}
