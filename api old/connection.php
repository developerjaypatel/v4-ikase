...
function getConnection() {
	...
	echo "mysql:host=$dbhost;dbname=$dbname", $dbuser, $dbpass;
	$dbh = new PDO("mysql:host=$dbhost;dbname=$dbname", $dbuser, $dbpass);            
	$dbh->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	return $dbh;
}

//solulab code start - 28-05-2019
function getConnectionIkaseMaster() {
	$dbhost = "ikase.org";
	$dbuser="root";
	$dbpass="admin527#";
	$dbname="ikase";
...
function getCustomerDocucentsAPIKey($cus_id){
	...
	try{		
		$dbConn =  getConnectionIkaseMaster();
...
function findDocumentFolder($customer_id, $case_id, $file, $type, $thumbnail_folder, $document_id) {
	...
	//https://v2.ikase.org/uploads/1042/6843/letters/1002%201ST%20THANK%20YOU%20LTR%20(1)_6843_0.docx
...
function create_zip($files = array(),$destination = '',$overwrite = false) {
	...
	if(is_array($files)) {
		...
		foreach($files as $file) {
			//replace the url
			$file = str_replace("https://v2.ikase.org", "..", $file);
