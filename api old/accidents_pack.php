<?php
$app->get('/accident', authorize('user'),	'getAccidents');
$app->get('/accident/:id', authorize('user'),	'getAccident');
$app->get('/accident/kase/:case_id', authorize('user'),	'getInjuryAccident');

$app->get('/disability/:id', authorize('user'),	'getDisability');
$app->get('/disability/injury/:injury_id', authorize('user'),	'getInjuryDisability');
$app->get('/disabilities/:injury_id', authorize('user'),	'getDisabilities');

//posts
//$app->post('/accident/delete', authorize('user'), 'deleteAccident');
$app->post('/accident/add', authorize('user'), 'addAccident');
$app->post('/disability/add', authorize('user'), 'addDisability');
$app->post('/disability/update', authorize('user'), 'updateDisability');
//$app->post('/accident/update', authorize('user'), 'updateAccident');

$app->get('/surgery/:id', authorize('user'), 'getSurgery');
$app->get('/surgeries/:case_id', authorize('user'), 'getKaseSurgeries');

$app->post('/surgery/save', authorize('user'), 'saveSurgery');
...
function addDisability() {
	...
	try {
		...
		*/
		...
		$sql = "INSERT INTO cse_case_disability (`case_disability_uuid`, `case_uuid`, `disability_uuid`, `attribute`, `customer_id`)
		VALUES ('" . $case_disability_uuid  ."', '" . $case_uuid . "', '" . $table_uuid . "', '" . addslashes($ailment) . "', '" . $_SESSION['user_customer_id'] . "')";
		//echo $sql;  
		$stmt = $db->prepare($sql);  
		$stmt->execute();
...
function addAccident() {
	...
	try {
		...
		$sql = "UPDATE `cse_injury_accident`
		SET deleted = 'Y'
		WHERE `injury_uuid` = '" . $injury_uuid . "'
		AND `customer_id` = '" . $_SESSION['user_customer_id'] . "'";
		$stmt = $db->prepare($sql);  
		...
		$sql = "INSERT INTO cse_injury_accident (`injury_accident_uuid`, `injury_uuid`, `accident_uuid`, `attribute`, `customer_id`)
		VALUES ('" . $injury_accident_uuid  ."', '" . $injury_uuid . "', '" . $table_uuid . "', 'main', '" . $_SESSION['user_customer_id'] . "')";
		//echo $sql;  
		$stmt = $db->prepare($sql);  
		$stmt->execute();
