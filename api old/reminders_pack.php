...
$app->get('/reminders/:event_id', authorize('user'),	'getReminders');
$app->get('/reminder/:id', authorize('user'),	'getReminder');

$app->post('/reminders/delete', authorize('user'), 'deleteReminder');
$app->post('/reminders/verify', authorize('user'), 'verifyReminder');
$app->post('/reminders/add', authorize('user'), 'addReminder');
$app->post('/reminders/update', authorize('user'), 'updateReminder');
$app->post("/reminders/newtime", authorize('user'), "getReminderDateTime");

$app->post('/reminders/statute', 'getStatuteReminders');
$app->post('/reminders/events', 'getEventReminders');
$app->post('/reminders/customerinvoices', 'getCustomerInvoiceReminders');
	...
	try {
		...
		foreach($reminders as $reminder) {
			if ($reminder->reminder_type == "interoffice" || $reminder->reminder_type == "email") {
				...
				$message = "The last invoice  for " . $reminder->cus_name . " <a href='https://v2.ikase.org/manage/customers/invoice.php?cus_id=" . $reminder->cus_id . "&invoice_id=" . $reminder->invoice_id . "'>" . $reminder->invoice_number . "</a> covered " . date("m/d/y", strtotime($reminder->start_date)) . " through " . date("m/d/y", strtotime($reminder->end_date));
				$message .= ".<br />";
				$message .= "<br />";
