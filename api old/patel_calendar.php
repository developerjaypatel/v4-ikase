...
$token="0b9e28a4975bf573f6a5f8ceec39ac17.TS54a2eeb959489";
...
function exportCalendar($token, $user_uuid) {	
	...
	try {
		...
		//$sql .=	" AND `eve`.`event_dateandtime` BETWEEN '2018-03-01' AND '2018-03-16' ";
		//$sql .=	" AND `eve`.event_id= 8943 ";
		$sql .=	" AND `eve`.`event_type` != 'intake' ";
		$sql .= " ORDER BY eve.event_id DESC
		LIMIT 0, 1000";
		//die($sql);
		$stmt = $db->prepare($sql);
		$stmt->execute();
		...
		if (count($allcusevents) > 0) {
			...
			if ($time_zone!="") {
				...
				$rightnow = mktime(date("H") + $offset, date("i"), date("s"), date("m"), date("d"),   date("Y"));
				//die(date("m/d/Y H:i:s", $rightnow));
			}
		}
...
if ($appointment->case_number!="") {
	$output .= "iKase Link: https://v2.ikase.org/v7.php#kases/" . $appointment->case_id;
	$output .= "\\n\\n";
...
if ($description!="") {
	...
	$last_two = substr($description, strlen($description) - 2);
	if ($last_two=="\\n") {
		$description = substr($description, 0, strlen($description) - 2);
	}
	//$description = cleanWord($description, true);
	//remove any leading returns
	/*
	$first_two = substr($description, 0, 2);
	if ($first_two=="\\n") {
