...
FROM cse_injury inj
INNER JOIN `cse_case` bc
ON inj.CASENO = bc.cpointer
INNER JOIN `cse_case_injury` bci
ON bc.case_uuid = bci.case_uuid
LEFT OUTER JOIN `cse_case_corporation` bcc
...
WHERE acc_emp.CASENO IS NULL
AND (inj.E_NAME != '' OR inj.E2_NAME != '')
AND bcc.case_corporation_id IS NULL
LIMIT 0, 100";
...
foreach($cases as $case_key=>$injury){
	...
	if ($injury->E_NAME!="") {
		...
		*/
		...
		INSERT INTO `cse_case_corporation` (`case_corporation_uuid`, `case_uuid`, `corporation_uuid`, `injury_uuid`, `attribute`, `last_updated_date`, `last_update_user`, `customer_id`)
	...
	if ($completed <  7351) {
		echo "<script language='javascript'>parent.runEmployers(" . $completed . ",7351)</script>";
	}
}
?>
</body>
