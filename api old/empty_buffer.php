...
function emptyBuffer($blnReminders = false) {
	...
	try { 		
		...
		$stmt = $db->prepare($sql);
		$stmt->execute();
		$buffer = $stmt->fetchAll(PDO::FETCH_OBJ);
		...
		if ($blnBlaster) {
			...
			if ($blnSendSMTP) {
				...
				if($tos!=""){
					...
					*/
					...
					foreach ($arrAttachments as $attachment_file) {
						...
						if ($blnInvoice) {
							...
							if ($case_id!="") {
								...
								if ($strpos == 0) {
									...
									$attachment_file = $attachment_dir . $case_id . "\\" . $plain_file;
									
									if (file_exists($attachment_dir . $case_id . "\\" . urldecode($plain_file))) {
						...
						//$attachment_file = "https://v2.ikase.org/" . $attachment_file;
				 ...
				 */
					...
					if (count($arrAttachments) > 0) {
						...
						foreach ($arrAttachments as $attachment_file) {
							if ($attachment_file!="") {
								//$attachment_file = $_SERVER["DOCUMENT_ROOT"] . "\\" . str_replace("/", "\\", $attachment_file);
			...
			if ($blnSent) {
				...
				if ($blnInvoice) {
					$db = getConnection();	
					$stmt = $db->prepare($sql_update_invoice);  
					$stmt->execute();
					$stmt = null; $db = null;
