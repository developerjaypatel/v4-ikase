<?php
$app->get('/letter/:id', authorize('user'),	'getLetter');

//posts
$app->post('/letter/delete', authorize('user'), 'deleteLetter');
$app->post('/letter/add', authorize('user'), 'addLetter');
$app->post('/letter/create', authorize('user'), 'createLetter');
$app->post('/letter/pcreate', authorize('user'), 'createLetterByPartieType');
$app->post('/letter/envelope', authorize('user'), 'createLetterEnvelope');
$app->post('/letter/penvelope', authorize('user'), 'createLetterEnvelopeByPartieType');
$app->post('/letter/updateheader', authorize('user'), 'createHeader');

$app->post('/letter/ready', authorize('user'), 'checkReadyLetter');

$app->post('/letter/customer', authorize('user'), 'createCustomerLetter');
$app->post('/letter/customerclean', 'cleanCustomerLetter');
$app->post('/letter/update', authorize('user'), 'updateLetter');
...
function checkReadyLetter() {
	...
	$destination_folder = "C:\\inetpub\\wwwroot\\iKase.org\\uploads\\" . $customer_id . "\\" . $case_id . "\\letters\\";
...
function createLetterEnvelope() {
	...
	*/
	...
	echo json_encode(array("file"=>$destination . ".docx"));
}
function createLetterEnvelopeByPartieType() {
	...
	$destination_folder = "C:\\inetpub\\wwwroot\\iKase.org\\uploads\\" . $customer_id . "\\envelopes\\";
...
function unlinkIt() {
	/*
	try {
		$filename = "C:\\inetpub\\wwwroot\\iKase.org\\uploads\\1111\\envelopes\\envelope_1034.docx";
		unlink($filename);
...
function createCustomerLetter() {
	...
	$url = "https://v2.ikase.org/api/letter/customerclean";
...
function createLetter() {
	...
	if ($blnProcessParty) {
		...
		if (count($any_partie) > 0) {	
			if (trim($any_partie->full_name)!="") {
	...
	try {
		...
		$stmt = $db->prepare($sql_personal_injury);
		$stmt->execute();
	...
	*/
	...
	if ($blnDefense) {		
		...
		$arrReplace['OPPCNAME1'] = $defense->full_name;
		// . ", ESQ"
	...
	*/
	...
	if ($template->document_filename=="kase_medindex.docx") {
		...
		$zip_source_path = 'C:\\inetpub\\wwwroot\\iKase.org\\uploads\\' . $customer_id . '\\' . $case_id . '\\';
		$zip_source_path = '../uploads/' . $customer_id . '/' . $case_id . '/';
		$zip_folder_path = 'C:\\inetpub\\wwwroot\\iKase.org\\uploads\\' . $customer_id . '\\zips\\' . $case_id . '\\';
...
", $rectype);
	...
	if ($signature_img!="") {
		//die($signature_img);
		$signature_img = "C:\\inetpub\\wwwroot\\iKase.org\\uploads\\" . $customer_id . "\\" . $signature_img;
	...
	*/
	...
	if ($kinv_data!="") {
		...
		if ($blnNormalReturn) {
			...
			$destination_folder_path = 'C:\\inetpub\\wwwroot\\iKase.org\\uploads\\' . $customer_id . '\\' . $case_id . '\\letters\\';
			$destination_path = $destination_folder_path . $destination;
			...
			if (count($arrExamAttach) > 0) {
				...
				//$attachments = str_replace("../", "https://v2.ikase.org/", $zip_path);
				$attachments = str_replace("../", "https://v2.ikase.org/", $zip_dir) . "/med_index_" . date("Ymd") . ".zip";
	...
	echo json_encode(array("success"=>$destination, "activity_id"=>$activity_id, "invoice_number"=>$kinvoice_number, "invoice_total"=>$invoice_total, "zip_path"=>$zip_path));
