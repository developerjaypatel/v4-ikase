...
$app->get('/contactmessages/:contact_id', authorize('user'), 'getContactMessages');
$app->get('/pendings', authorize('user'), 'getPendingMessages');
$app->get('/inbox', authorize('user'), 'getInbox');
$app->get('/inboxnew', authorize('user'), 'newInbox');
$app->get('/inboxday/:day', authorize('user'), 'dayInbox');
$app->get('/outboxday/:day', authorize('user'), 'dayOutbox');
$app->get('/inboxcheck', authorize('user'), 'checkInbox');
$app->get('/outbox', authorize('user'), 'getOutbox');
$app->get('/drafts', authorize('user'), 'getOutboxDrafts');
$app->get('/draftcount', authorize('user'), 'getDraftsCount');
$app->get('/messages/:message_id', authorize('user'), 'getMessage');
$app->get('/autoassign/:message_id/:customer_id', 'autoAssignMessage');
$app->get('/message/:message_id', authorize('user'), 'getMessageBody');

$app->get('/thread/pendings', authorize('user'), 'getThreadInboxPendings');
$app->get('/thread/inbox', authorize('user'), 'getThreadInbox');
$app->get('/thread/outbox', authorize('user'), 'getThreadOutbox');
$app->get('/threads/:thread_id', authorize('user'), 'getThreadBodies');

//$app->get('/phone_calls/:case_id', authorize('user'), 'getCalls');
//$app->get('/callsnew', authorize('user'), 'newPhoneCalls');

//posts
$app->post('/messages/read', authorize('user'), 'readMessage');
$app->post('/messages/delete', authorize('user'), 'deleteMessage');
$app->post('/feedback/add', 'addMessage');
$app->post('/messages/add', authorize('user'), 'addMessage');
$app->post('/messages/update', authorize('user'), 'updateMessage');
$app->post('/messages/add_email', 'addEmailMessage');
$app->post('/messages/confirm_email', authorize('user'), 'confirmEmailMessage');
$app->post('/messages/block_email', authorize('user'), 'blockEmailMessage');
$app->post('/messages/check_email', 'checkEmailMessage');
$app->post('/messages/setattachments', 'setMessageAttachments');

$app->get('/notifications', authorize('user'), 'getScanNotifications');

$app->post('/notification/add', authorize('user'), 'addNotification');

$app->post('/threads/delete', authorize('user'), 'deleteSpecificThread');
$app->post('/threads/unread', authorize('user'), 'unreadThread');
$app->post('/thread/confirm_email', authorize('user'), 'confirmEmailThread');
$app->post('/thread/block_email', authorize('user'), 'blockEmailThread');
...
(
	SELECT DISTINCT thr.thread_id `id`, thr.thread_uuid `uuid`, 
		...
		CONCAT(msg.message_to, ';', msg.message_cc, ';', msg.message_bcc) message_destination
	FROM cse_message msg
...
function checkEmailMessage($customer_id = "", $user_id="", $messageId = "", $from = "") {
	...
	if ($customer_id=="") {
		$blnInternal = false;
		if ($_SERVER['REMOTE_ADDR']!="173.58.194.150" && $_SERVER['REMOTE_ADDR']!="173.58.194.146" && $_SERVER['REMOTE_ADDR']!="173.58.194.148" && $_SERVER['REMOTE_ADDR']!="71.254.171.237") {
...
function addEmailMessage($arrMessage = "") {
	...
	if (count($arrMessage)==0) {
		if ($_SERVER['REMOTE_ADDR']!="173.58.194.150" && $_SERVER['REMOTE_ADDR']!="173.58.194.146" && $_SERVER['REMOTE_ADDR']!="173.58.194.148" && $_SERVER['REMOTE_ADDR']!="71.254.171.237" && $_SERVER['SERVER_NAME']!="v2.ikase.org") {
			echo $_SERVER['REMOTE_ADDR'] . "\r\n";
			die("no go...");
		}
		//die(print_r($_POST));
	...
	*/
	...
	try {				
		...
		$db = getConnection();
		$stmt = $db->prepare($sql);
		$stmt->bindParam("user_uuid", $user->user_uuid);
		$stmt->bindParam("customer_id", $customer_id);
		$stmt->execute();
		$recipient = $stmt->fetchObject();
		$stmt->closeCursor(); $stmt = null; $db = null;
		...
		`snippet`, `dateandtime`, `from`, `message_to`, `attachments`, `status`, `customer_id`) ";
		$sql .= " VALUES(:messageId, 'email', :messageBody, :messageSubject, :messageSnippet, :messageDate, :messageSender, :message_to, :attachments, 'created', :customer_id)";
...
function addMessage() {
	...
	if ($source_message_id != "") {
		if (strpos($source_message_id, "vservice")===false) {
			...
			$message .= "\r\n\r\nhttps://v2.ikase.org/down.php?key=" . $key;
		}
	}
...
function trackMessage($operation, $message_id, $blnFromEmail = false, $return = "") {
	...
	try {
		...
		$activity .= " ['" . $message->subject . "' sent on " . date("m/d/y h:iA", strtotime($message->dateandtime)) . "] was " . $operation . "  by " . $_SESSION['user_name'];
		...
		//if ($_SERVER['REMOTE_ADDR']=='47.153.49.248') {
			$url = "https://v2.ikase.org/api/messages/setattachments";
