...
window.Router = Backbone.Router.extend({
	...
	listEams: function (case_id) {
		...
		//	success: function(eamss) {
				...
				kase.set("no_uploads", true);
				$('#kase_content').html(new form_listing({collection: eamss, model: kase}).render().el);
				$("#kase_content").removeClass("glass_header_no_padding");
...
function composeLetter(element_id) {
	...
	/*
	if (template_id < 0) {
		var blank_letter = new Document({case_id: case_id});
		blank_letter.set("case_name", kase.name());
		$("#myModalBody").html(new letter_view({model: blank_letter}).render().el);
		$("#myModalLabel").html("New Letter");
		setTimeout(function() {
	...
	$("#modal_save_holder").html('<a title="Save Letter" class="interoffice save" onClick="saveModal()" style="cursor:pointer"><i class="glyphicon glyphicon-saved" style="color:#00FF00; font-size:20px">&nbsp;</i></a>');
...
function composeEams(element_id) {
	...
	/*
	if (template_id < 0) {
		var blank_eams = new Document({case_id: case_id});
		blank_eams.set("case_name", kase.name());
		$("#myModalBody").html(new eams_view({model: blank_eams}).render().el);
		$("#myModalLabel").html("New Eams");
		setTimeout(function() {
		$('.modal-dialog').css('top', '30%');
	}, 700);
	} else {*/
		//var template_eams = new Document({id: template_id, case_id: case_id});
		//template_eams.fetch({
			//success: function (data) {
...
function composeTask(element_id) {
	...
	if (task_id < 0) {
		...
		if (casing_file == "") {
			if (task_case_id == "" || typeof element_id == "undefined") {
				task_case_id == current_case_id
			}
			var kase = kases.findWhere({case_id: task_case_id});
			$("#case_fileInput").tokenInput("add", {id: task_case_id, name: kase.name()});
		}
...
var showAttachmentPreview = function(form_name, event, filename, case_id, customer_id) {
	...
	if (element!=null) {
		...
		for (var i = 0; i < arrayLength; i++) {
			filename = arrFiles[i];
			panel_html = "<div><a href='uploads/" + customer_id + "/";
