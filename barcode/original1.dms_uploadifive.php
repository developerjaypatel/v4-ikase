...
$uploadDir = $_SERVER['DOCUMENT_ROOT'] . "\\scans\\" . $customer_id;
//die($uploadDir);
if (!file_exists($uploadDir)) {
	mkdir($uploadDir, 0755, true);
}
$uploadDir = $_SERVER['DOCUMENT_ROOT'] . "\\scans\\" . $customer_id . "\\" . date("Ymd");
//die($uploadDir);
if (!file_exists($uploadDir)) {
	mkdir($uploadDir, 0755, true);
}
...
$uploadDir = '\\scans\\' . $customer_id . '\\' . date("Ymd") . '\\';
...
if (!empty($_FILES)) {
	...
	//foreach($arrStoredFiles as $stored_file) {				
		...
		try {
			...
			//die($sql); 
			$stmt = $db->prepare($sql);  
			$stmt->bindParam("filename", $stored_file);
