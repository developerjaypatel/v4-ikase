...
if ($cus_id>0) {
	...
	$letter_name = mysql_result($result, $int, "letter_name");
	...
	$import_db_source = mysql_result($result, $int, "import_db_source");
	...
	$db_result = mysql_query($db_query) or die("unable to run query<br />" .$db_query . "<br>" .  mysql_error());
	$no_of_db = mysql_numrows($db_result);
	$db_arr = array();
	for($i = 0; $i < $no_of_db; $i++) {
		array_push($db_arr, mysql_result($db_result, $i, "Database"));
	}
} else {
...
<script language="javascript">
	document.getElementById('import_db_source').onchange = function() {
		alert('Please be careful with this value. The selected one will be used as source database at the time of execution of import script!');
	};

	document.getElementById('submit').onclick = function() {
		var importDbSource = document.getElementById('import_db_source').value;
		var flag = 0;
		if(importDbSource == '') {
			document.getElementById('db_opt_err').innerHTML = 'Please select source database for import';
			flag = 1;
		}
		if(flag == 0) {
			var param = '';
			...
			for(var index = 0; index < tagName.length; index++) {
				var elems = document.forms["form1"].getElementsByTagName(tagName[index]);
				for(var i = 0; i < elems.length; i++) {
					if(elems[i].id != '' || elems[i].name != '') {
						if(elems[i].id != '') {
							param += elems[i].id + '=' + elems[i].value;
						} else if(elems[i].type == 'checkbox' && elems[i].checked == true) {
							param += elems[i].name + '=' + elems[i].value;
						} else {
							param += elems[i].name + '=' + '';
						}
