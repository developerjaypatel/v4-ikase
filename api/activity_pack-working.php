...
$app->group('', function (RouteCollectorProxy $app) {
	$app->group('/activity', function (RouteCollectorProxy $app) {
		...
		$app->get('/kases/{case_id}', 'getKaseActivities');
		$app->get('/kases/documents/{case_id}', 'getKaseDocuments');
		$app->get('/file_access/{case_id}', 'getKaseActivitiesNoFile');
		$app->get('/invoices/{case_id}', 'getKaseInvoices');
		$app->get('/billing/{case_id}', 'getKaseBilling');

		$app->get('/invoiceitem/{invoice_id}', 'getKaseInvoiceItems');
		$app->get('/invoiceitemfull/{invoice_id}', 'getKaseInvoiceItemsFull');
		$app->get('/kases/{case_id}/{invoice_id}', 'getKaseInvoice');

		$app->get('/billing/{case_id}/{kinvoice_id}', 'getActivityBilling');
		$app->get('/report/{user_id}/{start_date}/{end_date}', 'getReportActivities');
		$app->get('/summary/{start_date}/{end_date}', 'getReportSummary');
		$app->get('/archive/{case_id}', 'getKaseArchivedActivities');
		$app->get('/archivecount/{case_id}', 'getKaseCountArchivedActivities');
		$app->get('/stacks/{start_date}/{end_date}', 'stackActivity');
		$app->get('/billto/{case_id}', 'billTo');

		$app->get('/refvocational/{case_id}', 'hasVocationReferral');

		$app->post('/track', 'trackActivity');
		$app->post('/update_hours', 'updateHours');
		$app->post('/update_bulkhours', 'updateBulkHours');
		$app->post('/update_by', 'updateBy');

		$app->post('/insert_invoiceactivity', 'insertInvoiceActivity');
		$app->post('/insert_kinvoiceactivity', 'insertKInvoiceActivity');
		$app->post('/update_kinvoiceactivity', 'updateKInvoiceActivity');
	...
	$app->get('/singleactivity/{case_id}/{activity_id}', 'singleActivity');

	$app->post('/invoice/delete', 'deleteInvoice');

	$app->group('/rate', function (RouteCollectorProxy $app) {
		$app->get('/{id}', 'getRate');
		$app->post('/save', 'saveRate');
	});
	$app->get('/rates', 'getRates');
	$app->get('/ratebytype/{case_type}', 'getRateByCaseType');
...
(:corporation_kinvoice_uuid, :corporation_uuid, :kinvoice_uuid, :attribute, :right_now, :user_uuid, :customer_id)";
		...
		foreach($arrItemValues as $values) {
			DB::run($sql_insert . " VALUES " . $values);
		}
...
function createKInvoiceDocument($operation = "create", $kase, $case_id, $case_uuid, $kinvoice_uuid, $kinvoice_date, $kinvoice_number, $kinvoice_document_id, $invoice_total, $arrInvoiceItems, $arrInvoiceRates, $arrInvoiceMinutes, $arrInvoiceAmounts, $arrInvoiceDates, $carrier, $template_parent, $start_date, $end_date, $user_id, $customer_id) {
	...
	$ps_file = UPLOADS_PATH.'invoices'.DC.'pdf_' . $_SESSION["user_plain_id"] . '.ps1';
...
function getKaseActivities($case_id, $file_access = "N", $kinvoice_id = "N") {
	...
	*/
	...
	$sql = "SELECT DISTINCT `cse_activity`.`activity_id`, `cse_activity`.`activity_uuid`, 
			...
			`cse_activity`.`billing_date`, `cse_activity`.`billing_rate`, `cse_activity`.`billing_amount`, `cse_activity`.`billing_unit`, `cse_activity`.`ktags`,
			...
			#IFNULL(ci.invoice_id, '') invoice_id, IFNULL(ci.invoice_number, '') invoice_number
			...
			/*
			" . $join ." JOIN cse_invoice_activity cia
			ON `cse_activity`.`activity_uuid` = `cia`.`activity_uuid` AND cia.deleted = 'N'
			
			" . $join ." JOIN cse_invoice ci
	...
	*/
	...
	if ($blnFileAccess) {
		...
		'' `billing_date`, '' `billing_rate`, 0 `billing_amount`, '' `billing_unit`, '' `ktags`,  
		...
		$sql .= " 
		ORDER BY  `cse_activity`.`activity_date` DESC, `cse_activity`.`activity_id` DESC LIMIT 10";
	}
...
function getKaseDocuments($case_id) {
	if (!is_numeric($case_id)) {
		die();
	}
	$sql = "SELECT  v1.document_id as did, v1.document_name as dname FROM  
	ikase_reyes.cse_document as v1 inner join
	ikase_reyes.cse_case_document as v2 on v1.document_uuid =  v1.document_uuid
	inner join
	ikase_reyes.cse_case as v3 on v3.case_uuid =  v2.case_uuid
	 where v3.case_id = '" . $case_id . "' AND 
	 v3.customer_id = '" . $_SESSION['user_customer_id'] . "'  group by v1.document_id";
	...
	try {
		...
		if(isset($documents) && is_array($documents) && count($documents) > 0) {
			foreach($documents as $document) {
				$results[$document->dname] = $document->did;
			}
		}
...
function recordActivity($operation, $activity, $case_uuid, $track_id, $category = "", $billing_time = 0, $ktags_value = '') {
	try {
		$db = getConnection();
		...
		$sql = "INSERT INTO cse_activity (`activity_uuid`, `activity`, `hours`, `activity_category`, `activity_user_id`, `customer_id`, `ktags`)
		VALUES ('" . $activity_uuid . "', '" . addslashes($activity) . "', '" . $billing_time . "', '" . addslashes($category) . "', '" . $_SESSION['user_plain_id'] . "', " . $_SESSION['user_customer_id'] . ",'" . $ktags_value . "')";
		//echo $sql . "\r\n";
		DB::run($sql);
