...
$app->group('', function (\Slim\Routing\RouteCollectorProxy $app) {
	$app->group('/scrape', function (\Slim\Routing\RouteCollectorProxy $app) {
		$app->get('/{adj_number}', 'scrapeEams');
		$app->post('/save', 'scrapeSave');
		$app->post('/search', 'searchEams');
		$app->post('/update', 'updateSave');
	});

	$app->get('/state/{zip}', 'getState');
	$app->get('/searchqme/{scode}/{radius}/{zip}', 'scrapeQME');
...
function scrapeSave() {
	...
	$injury_id = DB::lastInsertId();
		...
		//echo $sql . "\r\n\r\n";
		$injury_bodyparts = DB::select($sql);
