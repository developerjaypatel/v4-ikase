...
function getEmail($user_id, $email_id, $blnActive = false, $blnReturn = false) {
	session_write_close();
	$sql = "SELECT e.*, cg.token, cue.user_email_id, cue.attribute emails_number, cuser.user_id, cuser.user_uuid, e.email_id id, e.email_uuid uuid
			FROM `cse_email` e
			...
			LEFT JOIN cse_gmail cg
			ON cue.user_email_id = cg.user_email_id
			WHERE 1
