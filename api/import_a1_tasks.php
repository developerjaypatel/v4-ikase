...
// NISHIT ADDED this on 11 march, 2021 due to prevent memory errors
ini_set("memory_limit", -1);
//set_time_limit(0);
...
try {
	...
	*/
	foreach($cases as $key=>$case){
		...
		// $sql= "SELECT CONCAT(FNAME,' ',LNAME) FROM `" . $GLOBALS['GEN_DB_NAME'] . "`.`staff` WHERE username = `" . $GLOBALS['GEN_DB_NAME'] . "`.`tasks`.WHOFROM";
		// $stmt = DB::run($sql);
		// $test_data = $stmt->fetchObject();
		// print_r($test_data);
		// // $test_data_data = $test_data->case_count;
		// echo "";
		// die();
		...
		IFNULL((SELECT CONCAT(FNAME,' ',LNAME) FROM `" . $GLOBALS['GEN_DB_NAME'] . "`.`staff` WHERE username = `" . $GLOBALS['GEN_DB_NAME'] . "`.`tasks`.WHOFROM),'LRS') `task_from`,
		IFNULL((SELECT CONCAT(FNAME,' ',LNAME) FROM `" . $GLOBALS['GEN_DB_NAME'] . "`.`staff` WHERE username = `" . $GLOBALS['GEN_DB_NAME'] . "`.`tasks`.WHOFROM),'LRS') `from`, IFNULL(`WHOTO`, '') `assignee`, IFNULL(`PRIORITY`, '') task_priority,
		...
		";
		// added ifnull condition in task_from and from columns
