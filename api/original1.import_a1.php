...
try {
	...
	if (isset($_GET["id"])) {
		$sql = "DELETE FROM  `ikase_" . $data_source . "`.`cse_case`
	...
	$db = getConnection(); $stmt = $db->prepare($sql); 
	$stmt->execute(); 
	$body_parts = $stmt->fetchAll(PDO::FETCH_OBJ); $stmt->closeCursor(); $stmt = null; $db = null;
...
VALUES ('" . $case_uuid . "', '" . $case->CASENO . "', '" . $case_number . "', '" . date("Y-m-d", strtotime($case->DATEENTER)) . "', '" . $casetype . "', 'LAO', '" . addslashes($case->CASESTAT) . "', '" . date("Y-m-d", strtotime($case->DATEENTER)) . "', '" . $case->ATTY_RESP . "', '" . $case->ATTY_HAND . "', '" . $case->SEC_HAND . "', " . $customer_id . ")";
		...
		if($case->CASESTAT != '' AND $case->CASETYPE != '') {
			...
			$db = getConnection(); $stmt = $db->prepare($sql);
			$stmt->execute();
			$casestatus = $stmt->fetchAll(PDO::FETCH_OBJ); $stmt->closeCursor(); $stmt = null; $db = null;
		...
		foreach($injuries as $injury_index=>$injury) {		
			...
			//die(print_r($injury));
			//doi dates
			if ($injury->DOI=="" || $injury->DOI=="0000-00-00") {
