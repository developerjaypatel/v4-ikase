...
try {
	...
	*/
	...
	$nickname = extractTag($data, "Nickname");
	$olp_per = extractTag($data, "OLPPer");
	$ol_per = extractTag($data, "OLPer");
	$signature = extractTag($data, "Signature");
	$sign_id = extractEntity($data, "SignsFor", "Associates");
	$cc_id = extractEntity($data, "CC", "Associates");

	//die();
	
	$sql = "INSERT INTO `gaylord`.`interim_user`
	(`original_id`, `status`, `name`, `nickname`, `type`, `title`, `olp_per`, `ol_per`, `signature`, `sign_id`, `cc_id`)
	VALUES (:id, :status, :name, :nickname, :type, :title, :olp_per, :ol_per, :signature, :sign_id, :cc_id);
	";
	...
	$stmt->bindParam("olp_per", $olp_per);
	$stmt->bindParam("ol_per", $ol_per);
	...
	$sql = "UPDATE `gaylord`.`dbo_associates`
	SET `LOCK` = 'done'
	WHERE `ID` = :id";
	...
	if ($case_count - $completed_count > 0) {	
		echo "<script language='javascript'>parent.runUsers(" . $completed_count . "," . $case_count . ")</script>";
	}
	die("all done");
