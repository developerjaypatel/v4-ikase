...
try {
	...
	//venues
	$sql = "select DISTINCT ccase.case_id, cuser.user_id oattorney, newuser.user_id attorney, newuser.customer_id
FROM ikase_reino.cse_case ccase
INNER JOIN ikase_reino.cse_user cuser
ON ccase.attorney = cuser.user_id
...
ORDER BY case_id";
	...
	foreach($cases as $case_key=>$case){
		...
		//now the kase
		$sql = "UPDATE ikase_reino.cse_case
		SET `attorney` = '" . $case->attorney . "'
