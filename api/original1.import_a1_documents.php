...
try {
	...
	$sql_truncate = "TRUNCATE `ikase_" . $data_source . "`.`cse_document`; 
	TRUNCATE `ikase_" . $data_source . "`.`cse_case_document`; ";
	...
	INNER JOIN `" . $GLOBALS['GEN_DB_NAME'] . "`.`doctrk1` `doc`
	...
	*/
	...
	foreach($cases as $key=>$case){
		...
		$sql = "INSERT INTO `ikase_" . $data_source . "`.`cse_case_document` (`case_document_uuid`, `case_uuid`, `document_uuid`, `attribute_1`, `last_updated_date`, `last_update_user`, `deleted`, `customer_id`)
		...
		FROM `" . $GLOBALS['GEN_DB_NAME'] . "`.`doctrk1`
		WHERE `CASENO` = '" . $case->cpointer . "'";
		echo $sql . "\r\n\r\n";
		$stmt = $db->prepare($sql);
		$stmt->execute();
		
		$sql = "INSERT INTO `ikase_" . $data_source . "`.`cse_document` (document_uuid, parent_document_uuid, document_name, document_date, type, `description`, `description_html`, customer_id) 
