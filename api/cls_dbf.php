                ...
                if($Field["Type"] == "M" and $Field["Size"] == 4 and !empty($RawData)) {
					...
					if ($Field["Type"] == "I") {
						...
						for ($i = 1; $i < $Field["Size"]; $i++) {
							$theval = substr($RawData, $i - 1, 1);
							//echo "theval:" . $theval . "\r\n";
							$choppedData[] = $theval;
						}
						
						if ($Field["Name"]=="CASENO") {
							//echo "byL:" . $Field["Bytepos"]  . "\r\n";
							//echo "raw:" . $RawData . " len:" . strlen($RawData) . "\r\n";
							//die(print_r($choppedData));
						}
						
						//$RawData = substr($RawData, 0, 1);
						$Value = ord($choppedData[0]);
						//$Value = ord($RawData);
						//echo $choppedData . " -> init:" . $Value . "\r\n";
						
						for ($i = 1; $i < count($choppedData); $i++) {
							$Value += $i * 256 * ord($choppedData[$i]);
							//echo " -> val:" . $Value . "\r\n";
						}
					}
                }
				if ($Field["Name"]=="ACTNO" || $Field["Name"]=="CASENO" || $Field["Name"]=="EVENT") {		
					if($FieldCaptions) {
