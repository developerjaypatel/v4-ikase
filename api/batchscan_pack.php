...
function stitchBatchscanStacks() {
	...
	foreach($arrStack as $stack) {
		...
		$fileKaseFolder = checkFileExist($accessToken, "name='batchscan' and '".$iKaseFolderId."' in parents");
		$fileKaseFolderId = $fileKaseFolder['files'][0]['id'];

		$caseFileNm = $uploaded.'.pdf';
		...
		RenameFile($accessToken, $ikaseFileId, $document_filename);
...
//Google Drive Implementation 2022-02-15
function RenameFile($accessToken, $ikaseFileId, $ikase_fileName){
	$ch = curl_init();
	$data = '{"name":"'.$ikase_fileName.'"}';
	$options = [
		CURLOPT_URL =>  "https://www.googleapis.com/drive/v3/files/".$ikaseFileId,
...
//#END# Google Drive Implementation 2022-02-15
