...
try {
	...
	if (isset($_GET["reset"])) {
		...
		TRUNCATE `ikase_" . $data_source . "`.`cse_event_user`;";
		//echo $sql_truncate . "\r\n\r\n";
	...
	INNER JOIN `" . $GLOBALS['GEN_DB_NAME'] . "`.`cal1`
	ON mc.cpointer = `cal1`.`CASENO`
	WHERE 1
	AND mc.case_uuid NOT IN (SELECT DISTINCT case_uuid FROM `ikase_" . $data_source . "`.`cse_case_event`)
	LIMIT 0, 1";
	echo $sql . "\r\n\r\n";
	$stmt = $db->prepare($sql);
	$stmt->execute();
	...
	*/
	...
	FROM `" . $GLOBALS['GEN_DB_NAME'] . "`.`cal1` gcase
	WHERE 1";
	echo $sql . "\r\n<br>";
	//die();
	$stmt = $db->prepare($sql);
	...
	FROM `ikase_" . $data_source . "`.`cse_case_event` ggc
	WHERE 1";
	echo $sql . "\r\n<br>";
	//die();
	$stmt = $db->prepare($sql);
