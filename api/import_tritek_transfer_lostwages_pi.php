<?php
require_once('../shared/legacy_session.php');

include("connection.php");

try {
	$db = getConnection();
	
	//lookup the customer name
	include("customer_lookup.php");
	
	$sql = "INSERT INTO `ikase_" . $data_source . "`.`cse_case_lostincome`
	(`case_lostincome_uuid`, `case_uuid`, `lostincome_uuid`, `attribute`, `last_updated_date`, 
	`last_update_user`, `deleted`, `customer_id`)
	SELECT `case_lostincome_uuid`, ccn.`case_uuid`, `lostincome_uuid`, `attribute`, `last_updated_date`, 
	`last_update_user`, `deleted`, `customer_id` 
	FROM `" . $data_source . "`.`" . $data_source . "_case_lostincome`  ccn 
	INNER JOIN " . $data_source . ".badcases
	ON ccn.case_uuid = badcases.case_uuid 
	WHERE 1";

	echo $sql . "\r\n\r\n";
	//$stmt = $db->prepare($sql);
	//$stmt->execute();
	
	$sql = "INSERT INTO `ikase_" . $data_source . "`.`cse_lostincome` (`lostincome_uuid`, `start_lost_date`, `end_lost_date`, `comments`, `amount`, `deleted`, `customer_id`)
	SELECT ev.`lostincome_uuid`, `start_lost_date`, `end_lost_date`, `comments`, `amount`, ev.`deleted`, ev.`customer_id` 
	FROM `" . $data_source . "`.`" . $data_source . "_lostincome`  ev
	INNER JOIN `ikase_" . $data_source . "`.`cse_case_lostincome` cce
	ON ev.lostincome_uuid = cce.lostincome_uuid
	INNER JOIN " . $data_source . ".badcases
	ON cce.case_uuid = badcases.case_uuid  
	WHERE 1";
	
	echo $sql . "\r\n\r\n";
	//$stmt = $db->prepare($sql);
	//$stmt->execute();
	
	$success = array("success"=> array("text"=>"done @" . date("H:i:s")));
	echo json_encode($success);
} catch(PDOException $e) {
	$error = array("error"=> array("text"=>$e->getMessage()));
	echo json_encode($error);
}	
//, `notedesc`, , `notepoint`, `notelock`, `lockedby`, `xsoundex`, `locator`, `enteredby`, `mailpoint`, `lockloc`, `docpointer`, `doctype`
include("cls_logging.php");
?>
<script language="javascript">
parent.setFeedback("lostincome transfer completed");
</script>
