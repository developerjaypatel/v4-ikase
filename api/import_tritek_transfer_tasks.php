...
try {
	...
	//die();
	/*
	$sql = "DELETE FROM `ikase_" . $data_source . "`.`cse_case_task` 
	WHERE customer_id = " . $customer_id . "
	";
	$stmt = DB::run($sql);
	
	$sql = "DELETE FROM `ikase_" . $data_source . "`.`cse_task` 
	WHERE customer_id = " . $customer_id . "
	";
	$stmt = DB::run($sql);
	
	$sql = "DELETE FROM `ikase_" . $data_source . "`.`cse_task_user` 
	WHERE customer_id = " . $customer_id . "
	";
	$stmt = DB::run($sql);
	*/
	
	$sql = "TRUNCATE `ikase_" . $data_source . "`.`cse_case_task` 
	";
	$stmt = DB::run($sql);
	
	$sql = "TRUNCATE  `ikase_" . $data_source . "`.`cse_task` 
	";
	$stmt = DB::run($sql);
	
	$sql = "TRUNCATE `ikase_" . $data_source . "`.`cse_task_user` 
	";
	$stmt = DB::run($sql);
	...
	`last_update_user`, `deleted`, `customer_id` 
	FROM `" . $data_source . "`.`" . $data_source . "_case_task` 
