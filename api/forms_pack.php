...
function getForms() {
	...
	{	
		// For demo customer retrieves data from own DB only		
	...
	{
		// For other than demo customer, retrieves data from own DB and ikase DB. So using UNION...
		$sql = "SELECT *, `eams_form_id` `id`, '' ikasedb_eamsformid, '1' customer_id 
		FROM `cse_eams_forms` 
		WHERE 1
		AND (customer_id = " . $_SESSION['user_customer_id'] . " OR customer_id = 0)
		AND deleted = 'N'";

		$sql .= " UNION SELECT *, `eams_form_id` `id`, '-1' customer_id 
