...
foreach($arrRows as $key=> $row) {
	...
	if (is_numeric($arrRow[0])) {
		...
		if ($numbs==0) {
			$query_add = "INSERT INTO `cse_eams_carriers` (`eams_ref_number`, `firm_name`, `street_1`, `street_2`, `city`, `state`, `zip_code`, `phone`, `service_method`, `last_update`)
	VALUES ";
			if ($arrRow[0]!="") {
			...
			$query_add = "UPDATE `cse_eams_carriers` 
