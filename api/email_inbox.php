...
function assignGmail() {
	...
	// checking for auto or manual case assign
	if(strpos($case_attach,";") > 0)
	{
	...
	$new_id = DB::lastInsertId();
		...
		*/
		...
		$sql = "UPDATE cse_message mes
		SET mes.`subject` = CONCAT('" . addslashes($message_prefix) . "', mes.`subject`)
		WHERE 1
		AND mes.message_id = :id
		AND mes.customer_id = '" . $_SESSION["user_customer_id"] . "'
		AND INSTR(mes.`subject`, '" . addslashes($message_prefix) . "') = 0";
