...
$app->group('', function (RouteCollectorProxy $app) {
	$app->group('/kase', function (RouteCollectorProxy $app) {
		$app->get('/corporation/{case_id}/{corporation_id}', 'getKaseCorporation');
		$app->get('/corporationbyid/{corporation_id}/{case_id}', 'getKaseCorporationByID');
	});

	$app->get('/prior_treatments/{person_id}', 'getPriorTreatments');
	$app->get('/employees/tokeninput/{id}', 'getTokenCorporationEmployees');
	$app->get('/corporationinfo/{id}', 'getCorporationInfo');
	$app->get('/employers', 'getEmployers');
	$app->get('/corpcasecount/{corporation_id}/{type}', 'getCorporationCaseCount');

	$app->group('/lostincome', function (RouteCollectorProxy $app) {
		$app->get('/{id}', 'getLostIncome');
		$app->post('/save', 'addLostIncome');
		$app->post('/delete', 'deleteLostIncome');
	});
	$app->get('/kase/lostincome/{case_id}', 'getKaseLostIncome');
	$app->get('/kase/lostincometotal/{case_id}', 'getKaseLostIncomeTotal');
	$app->get('/lostincome/{case_id}/{corporation_id}', 'getEmployerLostIncome');

	$app->post('/judges', 'getJudges');

	$app->group('/corporation', function (RouteCollectorProxy $app) {
		$app->get('/{type}/{id}', 'getCorporation');
		$app->get('/kases/{id}', 'getCorporations');
		$app->post('/delete', 'deleteCorporation');
		$app->post('/import', 'importCorporation');
		$app->post('/add', 'addCorporation');
		$app->post('/update', 'updateCorporation');

		$app->post('/field/update', 'updateCorporationField');
		$app->get('/tokeninput/{type}', 'getTokenCorporations');
		$app->get('/recipient/{search_term}/{case_id}/{type}', 'getRecipients');
	});
	$app->post('/corporationkai/update', 'updateCorporationKai');

	$app->post('/qme_check', 'qmeCheck');
...
function getCorporationInfo($id) {
	...
	//die($sql);
    return DB::runOrApiError($sql, ['id' => $id])->fetchObject();
}
		...
		if  ($fieldname=="corporation_id") {
			...
			if (is_numeric($value) && $blnWCAB) {
				if ($value > -1) {
					...
					// || count($parent_corporation)==0
					if (!is_object($parent_corporation)) {
						$parent_corporation = getCorporationInfo($value);
					}
					//die(print_r($parent_corporation));
