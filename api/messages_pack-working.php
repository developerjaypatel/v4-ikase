...
$app->group('', function (RouteCollectorProxy $app) {
	...
	$app->group('/ktags', function (RouteCollectorProxy $app) {
		$app->get('/filters', 'ktagsFilter');
		$app->post('/add', 'ktagsAdd');
		$app->post('/update', 'ktagsUpdate');
	});
...
function addEmailMessage($arrMessage = "") {
	...
	if (count($arrMessage)==0) {
		...
		$subject = preg_replace("/\s+/", " ", $subject); // removing multiple blank spaces
		...
		// $message_body = urldecode($_POST["messageBody"] . "<br>" . $subject);
		...
		$snippet = $arrMessage["messageSubject"];
		$attachments = $arrMessage["attachments"];
	...
	*/
	...
	try {				
		...
		if ($case_id!="") {
			...
			// commented and added by mukesh

			$subject = "RE: " . $kase->name . " // ID " . $kase->id . "	" . $subject;
...
function addMessage() {	
	...
	foreach($_POST as $fieldname=>$value) {
		...
		*/
		...
		// when ktags field get in post call		
		if($fieldname=="ktags_dropdown_list")
		{
			$ktags_value = $value;			
		}
		else
...
`customer_id`)
		...
		//track now
		trackMessage($operation, $new_id, false,"", $ktags_value);	
	} catch(PDOException $e) {	
...
function trackMessage($operation, $message_id, $blnFromEmail = false, $return = "", $ktags_value = null) {
...
function ktagsFilter() {
	session_write_close();
	$sql = "SELECT * FROM ikase.cse_activity_ktags WHERE customer_id = " . $_SESSION['user_customer_id'];
	//die($sql);
	try {
		$db = getConnection();
		$stmt = $db->query($sql);
		$ktags = $stmt->fetchAll(PDO::FETCH_OBJ);
		
		echo json_encode($ktags);
	} catch(PDOException $e) {
...
function ktagsAdd() {
	session_write_close();
	$ktags = passed_var("ktags", "post");
	$ktags = str_replace(" ", "_", $ktags);

	$sql = "INSERT INTO ikase.cse_activity_ktags  (`activity_ktags`,`customer_id`) VALUES ('$ktags', '$_SESSION[user_customer_id]')";
	try {		
		$db = getConnection();
		$stmt = $db->prepare($sql);
		$stmt->execute();

		echo json_encode(array("success"=>true, "ktags_id"=>"1"));
	} catch(PDOException $e) {
		$error = array("error"=> array("text"=>$e->getMessage()));
        	echo json_encode($error);
	}
...
function ktagsUpdate() {
	session_write_close();
	$ktags_id = passed_var("ktags_id", "post");
	$ktags = passed_var("ktags", "post");
	$deleted = passed_var("deleted", "post");
	$ktags = str_replace(" ", "_", $ktags);
	if($deleted == "N")
	{
		$sql = "UPDATE ikase.cse_activity_ktags SET activity_ktags = '$ktags' WHERE tag_id = $ktags_id";
	}
	else
	{
		$sql = "DELETE FROM ikase.cse_activity_ktags WHERE tag_id = $ktags_id";
	}
	try {		
		$db = getConnection();
		$stmt = $db->prepare($sql);
