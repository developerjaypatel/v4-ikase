...
function saveStatusFilters() {
	...
	$sql = "INSERT INTO `cse_" . $table_name . "` (" . $table_name . "_uuid, " . $table_name . ", last_change_user, last_change_date)
	SELECT '" . $table_uuid . "', :casestatus, :user_uuid, :right_now
						FROM dual
						WHERE NOT EXISTS (
							SELECT * 
							FROM `cse_" . $table_name . "` 
							WHERE " . $table_name . " = :casestatus
						)";
	//die($sql);
	try {
		$db = getConnection();
