...
FROM `" . $data_source . "`.injury inj
INNER JOIN `" . $data_source . "`.`" . $data_source . "_case` bc
ON inj.CASENO = bc.cpointer
INNER JOIN `" . $data_source . "`.`" . $data_source . "_case_injury` bci
ON bc.case_uuid = bci.case_uuid
LEFT OUTER JOIN `" . $data_source . "`.`" . $data_source . "_case_corporation` bcc
...
foreach($cases as $case_key=>$injury){
	...
	if ($injury->E_NAME!="") {
		...
		INSERT INTO `" . $data_source . "`.`" . $data_source . "_corporation` 
		(`corporation_uuid`, `customer_id`, `company_name`, `type`, `full_address`, 
			`street`, `city`, `state`, `zip`, `phone`, `fax`,  
			`last_updated_date`, `last_update_user`, `deleted`, `parent_corporation_uuid`, `copying_instructions`) 
		...
		*/
		...
		INSERT INTO `" . $data_source . "`.`" . $data_source . "_case_corporation` (`case_corporation_uuid`, `case_uuid`, `corporation_uuid`, `injury_uuid`, `attribute`, `last_updated_date`, `last_update_user`, `customer_id`)
