...
try {
	...
	foreach($cases as $case_key=>$case){
		...
		//now the kase
		$sql = "INSERT INTO `" . $data_source . "`.`" . $data_source . "_case` (case_uuid, cpointer, case_number, case_date, case_type, venue, case_status, submittedOn, supervising_attorney, attorney, worker, customer_id) 
VALUES ('" . $case_uuid . "', '" . $case->CASENO . "', '" . $case_number . "', '" . date("Y-m-d", strtotime($case->DATEENTER)) . "', '" . $case->CASETYPE . "', 'LAO', '" . addslashes($case->CASESTAT) . "', '" . date("Y-m-d", strtotime($case->DATEENTER)) . "', '" . $case->ATTY_RESP . "', '" . $case->ATTY_HAND . "', '" . $case->SEC_HAND . "', " . $customer_id . ")";
