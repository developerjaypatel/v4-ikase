...
function getReportActivities($user_id, $start_date, $end_date) {
	...
	*/
	//die($sql);
	$sql1 = "SELECT IFNULL(user.nickname, '') `by`, IFNULL(user.user_name, '') `user_name`
	FROM cse_docucents_track 
	LEFT OUTER JOIN cse_case_activity 
    ON `cse_case_activity`.last_update_user = `cse_docucents_track`.`user_uuid`
	LEFT OUTER JOIN ikase.cse_user `user`
    ON `cse_docucents_track`.user_uuid = user.user_uuid
	WHERE timestamp BETWEEN '" . $start_date . " 00:00:00' AND '" . $end_date . " 23:59:59'
	AND user_role != 'masteradmin'
	GROUP BY docucents_track_id
	";
	//echo $sql1; exit;
	
	//$_SESSION["docucent_activity_sql"] = $sql1;
	$_SESSION["last_activity_sql"] = $sql;
	session_write_close();
	...
	try {
		...
		$stmt1->execute();
		$docucentactivities = $stmt1->fetchAll(PDO::FETCH_OBJ);
		//echo '<pre>'; print_r($docucentactivities); exit;
		$activities= $stmt->fetchAll(PDO::FETCH_OBJ);
		$stmt->closeCursor(); $stmt = null; $db = null;
