...
function checkEmailMessage($customer_id = "", $user_id="", $messageId = "", $from = "") {
	...
	if ($customer_id=="") {
		...
		if ($_SERVER['REMOTE_ADDR']!="47.181.68.46" && $_SERVER['REMOTE_ADDR']!="173.58.194.146" && $_SERVER['REMOTE_ADDR']!="173.58.194.148" && $_SERVER['REMOTE_ADDR']!="71.254.171.237" && $_SERVER['SERVER_NAME']!="v4.ikase.org") {
...
function addEmailMessage($arrMessage = "") {
	...
	if (count($arrMessage)==0) {
		// NISHIT REPLACE IP FROM 173.58.194.150 TO 47.181.68.46
		if ($_SERVER['REMOTE_ADDR']!="47.181.68.46"  
	...
	if (is_object($message)) {
		...
		if ($message_type=="email") {
			...
			if(strpos($email_info->email_name, "@gmail.com") === false && $email_info->read_message=="Y") {
				...
				// NISHIT REPLACE IP FROM 173.58.194.150 TO 47.181.68.46
				$url = "http://47.181.68.46/ikase/limapi/email.php/seemail";
