...
$app->group('', function (\Slim\Routing\RouteCollectorProxy $app) {
	$app->group('/user', function (\Slim\Routing\RouteCollectorProxy $app) {
		...
		$app->get('/events/{event_id}/{type}', 'getEventUsers');
		$app->get('/messages/{message_id}/{type}', 'getMessageUsers');
		$app->get('/tasks/{task_id}/{type}', 'getTaskUsers');
		$app->get('/search/{search_term}', 'findUsers');
		$app->get('/transferwork/{from_id}/{to_id}', 'transferWork');

		$app->post('/delete', 'deleteUser');
		$app->post('/imei', 'imeiUser');
		$app->post('/add', 'addUser');
		$app->post('/update', 'updateUser');

		//login and activity track
		$app->get('/tracksummary/{id}', 'loginTrackSummary');
		$app->get('/tracksummarybydate/{id}/{start_date}/{end_date}', 'loginTrackSummaryByDate');
	});

	$app->get('/currentusers', 'getCurrentUsers');
	$app->get('/contact', 'getContacts');
	$app->get('/users', 'getAllUsers');
	$app->get('/usercontact', 'getMessageContacts');
	$app->get('/attorney', 'getAttorneys');

	$app->get('/userinfo/{id}', 'getUserInfo');
	$app->get('/usernickname/{nickname}', 'getUserByNickname');
	$app->get('/fetchnickname/{nickname}', 'fetchUserByNickname');
	$app->get('/anytime', 'verifyAnytime');

	$app->get('/contact/search/{search_term}', 'findMessageUsers');

	$app->post('/rankpassword', 'rankPassword');
...
function performReset() {
	...
	try {
		$db = getConnection();
		$stmt = $db->prepare("UPDATE ikase.`cse_user` `user`
				SET user.`pwd` = '$password'
				WHERE `user_id` = :id");
		$stmt->bindParam("id", $id);
		$stmt->execute();
		return ["success" => "password updated", 'affected' => $stmt->rowCount(), 'pwd' => $password];
	}
	catch (PDOException $e) {
...
function findMessageUsers($search_term, $blnOnlyActive = true) {
	/*// user_name, user_first_name, user_last_name, user_name as name
    $sql = "SELECT user.user_id, user.user_uuid, user.user_email as user_name, user.user_logon, user.user_first_name, user.user_last_name, user.nickname, `user`.user_email, `user`.`dateandtime`, `user`.`status`, `user`.`calendar_color`, `user`.`personal_calendar`, `user`.access_token, `user`.user_type, IF(`user`.user_type='1', 'admin', 'user') `role`, `user`.user_id id, `user`.user_uuid uuid, job.job_id, job.job_uuid, if(job.job IS NULL, '', job.job) `job`, `user`.`user_email` `name`
			FROM ikase.`cse_user` user 
			LEFT OUTER JOIN ikase.`cse_user_job` cjob
			ON (user.user_uuid = cjob.user_uuid AND cjob.deleted = 'N')
			LEFT OUTER JOIN `cse_job` job
	...
	$sql .= " WHERE user.deleted = 'N'
			...
			OR user.user_name LIKE '%" . addslashes($search_term) . "%'
			OR user.nickname LIKE '%" . addslashes($search_term) . "%'
			OR user.user_email LIKE '%" . addslashes($search_term) . "%'
			)";
	if ($blnOnlyActive) {
		$sql .= " 
			AND user.activated = 'Y'";
