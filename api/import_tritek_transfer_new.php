...
SELECT ccase.`case_uuid`, `case_number`, ccase.`cpointer`, `case_name`, 'tritek', `adj_number`, `case_date`, `case_type`, `venue`, `dois`, `case_status`, `case_substatus`, `rating`, `submittedOn`, `attorney`, `worker`, `deleted`, `customer_id`, `medical`, `td`, `rehab`, `edd` 
FROM `" . $data_source . "`.`" . $data_source . "_case` ccase
INNER JOIN `" . $data_source . "`.`badcases` 
ON ccase.case_uuid = badcases.case_uuid
WHERE 1 
AND case_type != 'WCAB'
AND INSTR(case_type, 'Workers') = 0
