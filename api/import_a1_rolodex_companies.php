...
try {
	...
	$sql = "SELECT `ac`.`CARDCODE`,  `ac`.`FIRMCODE`,  `ac`.`LETSAL`,  
		...
		FROM `" . $GLOBALS['GEN_DB_NAME'] . "`.card ac
        INNER JOIN `" . $GLOBALS['GEN_DB_NAME'] . "`.card2 ac2
		ON ac.FIRMCODE = ac2.FIRMCODE
		LEFT OUTER JOIN `" . $GLOBALS['GEN_DB_NAME'] . "`.casecard acc
		ON ac.CARDCODE = acc.CARDCODE
		LEFT OUTER JOIN `" . $GLOBALS['GEN_DB_NAME'] . "`.card3 card3
	...
	foreach($parties as $key=>$partie){	
		...
		if ($type=="applicant" || $type=="client" || $type=="mailing_list") {		
			...
			if (is_object($rolodex)) {
				...
				WHERE `cse_card`.`CARDCODE` = '" . $CARDCODE . "'";
