...
$app->group('', function (\Slim\Routing\RouteCollectorProxy $app) {
	...
	$app->post('/pdf/create', 'generatePDF');
})->add(\Api\Middleware\Authorize::class);
$app->get('/pdf/test', 'testPDF');
//putenv("MAGICK_HOME=C:\\Program Files\\ImageMagick-7.1.1-Q16");
//putenv("GS_LIB=C:\\Program Files\\gs\\gs9.15\\bin");

function testPDF() {
	//$bat = UPLOADS_PATH."1064\\4883\\eams_forms\\pdftk.bat";
...
Directory Listing for `uploads\\1064\\4883\\eams_forms\\`:<br />";
	...
	$dir = UPLOADS_PATH."1064\\4883\\eams_forms\\";
	$files = scandir($dir);
	$blnFound = false;
...
//		die($somecontent );
	...
	*/
	...
	$filename = UPLOADS_PATH . $_SESSION['user_customer_id'] . "\\envelopes\\" . $form_name . "_plain.fdf";
	$pdftk_output =  UPLOADS_PATH . $_SESSION['user_customer_id'] . "\\envelopes\\" . $form_name . "_" . $corporation_id . ".pdf";
	$file_counter = 1;
...
function generatePDF($form_name = "", $separator_title = "") {
	...
	try {
		$all_claim_numbers = DB::select($sql_claims);
	...
	$pdftk_output =  UPLOADS_PATH . $_SESSION['user_customer_id'] . DC . $case_id . "\\eams_forms\\" . $form_file_name . ".pdf";
	//$pdftk_output =  "D:\\uploads\\" . $_SESSION['user_customer_id'] . DC . $case_id . "\\eams_forms\\" . $form_file_name . "_" . $file_counter . ".pdf";
	$file_counter = 1;
	$output_store_name = $form_file_name . ".pdf";
	while (file_exists($pdftk_output)) {
		$pdftk_output =  UPLOADS_PATH . $_SESSION['user_customer_id'] . DC . $case_id . "\\eams_forms\\" . $form_file_name . "_" . $file_counter . ".pdf";
	...
	//$filename = "D:\\uploads\\" . $_SESSION['user_customer_id'] . DC . $case_id . "\\eams_forms\\" . $form_file_name . ".fdf";
	$filename = str_replace("C:\inetpub\wwwroot\ikase.org\uploads", "C:\inetpub\wwwroot\uploads", $filename);

	$pdftk_output = str_replace("C:\inetpub\wwwroot\ikase.org\uploads", "C:\inetpub\wwwroot\uploads", $pdftk_output);
	//C:\inetpub\wwwroot\ikase.org\uploads\1033\278\eams_forms\4906H.fdf
	//echo "pdftk_output".$pdftk_output;
	$source_dir = $_SERVER['DOCUMENT_ROOT'] . '\\eams_forms\\';
	//$source_dir = str_replace("C:\inetpub\wwwroot\ikase.org\eams_forms\\", "C:\inetpub\wwwroot\uploads\eams_forms\\", $source_dir);
	//$source_dir = "D:\\ikase.org\\eams_forms\\";
	if ($nopublish=="y") {
		if ($case_id==1) {
			//die("pdftk " . $source_dir . $form_name . ".pdf fill_form " . $filename. " output " . $pdftk_output);
		}
		//echo "pdftk \"" . $source_dir . $form_name . ".pdf\" fill_form \"" . $filename. "\" output \"" . $pdftk_output . "\"";die;
		exec("pdftk \"" . $source_dir . $form_name . ".pdf\" fill_form \"" . $filename. "\" output \"" . $pdftk_output . "\"");//die;
	

		//exec('pdftk "D:\ikase.org\eams_forms\4906H.pdf" fill_form "D:\uploads\1033\278\eams_forms\4906H.fdf" output "D:\uploads\1033\278\eams_forms\4906H.pdf"');
	...
	if ($form_name!="app_cover" && !$blnInternalRequest) {
		...
		$upload_dir = str_replace("C:\inetpub\wwwroot\ikase.org\uploads", "C:\inetpub\wwwroot\uploads", $upload_dir);
		//echo $upload_dir;die;
	...
	if ($form_name=="app_cover") {
		...
		*/
		...
		foreach($documents as $doc_index=>$doc) {
			...
			//UPLOADS_PATH . $_SESSION['user_customer_id'] . DC . $case_id . "\
		}
		//die(print_r($arrFiles));
		//put together the pdftk string
		$filename_output = UPLOADS_PATH . $_SESSION['user_customer_id'] . DC . $case_id . "\jetfiler\\app_cover.pdf";
		
		$file_counter = 1;
		$output_store_name = $form_name . "_final.pdf";
		$pdftk_output =  UPLOADS_PATH . $_SESSION['user_customer_id'] . DC . $case_id . "\\eams_forms\\" . $output_store_name;
		
		$upload_dir = UPLOADS_PATH . $_SESSION['user_customer_id'] . DC . $case_id;
		...
		*/
		...
		while (file_exists($pdftk_output)) {
			$pdftk_output =  UPLOADS_PATH . $_SESSION['user_customer_id'] . DC . $case_id . "\\eams_forms\\" . $form_name . "_" . $file_counter . "_final.pdf";
		...
		//, "pdftk"=>$pdftk
		//$destination_folder = "D:/uploads/" . $_SESSION['user_customer_id'] . "/" . $case_id  . "/eams_forms/";
		$destination_folder = "C:/inetpub/wwwroot/uploads/" . $_SESSION['user_customer_id'] . "/" . $case_id  . "/eams_forms/";
		echo json_encode(array("file"=>$destination_folder . $output_store_name, "activity_id"=>""));
