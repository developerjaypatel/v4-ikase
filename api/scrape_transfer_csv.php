...
function getKaseInfo($id, $worker_name, $adj_number, $courtcalendar_id, $customer_id, $db_name) {
...
$sql .= " app ON ccapp.person_uuid = app.person_uuid
	...
	try {
		...
		if (!is_object($kase)) {
			$blnReturnNew = true;
			$case_uuid = uniqid("CS");
			$case_name = $worker_name;
			
			$sql = "INSERT INTO `court_calendar`.cse_case (case_uuid, case_name, adj_number, customer_id)
		SELECT :case_uuid, :case_name, :adj_number, :customer_id
		FROM dual
						WHERE NOT EXISTS (
							SELECT * 
							FROM `court_calendar`.cse_case
							WHERE case_name = :case_name
							AND adj_number = :adj_number
		...
		if ($blnReturnNew) {	
			...
			$kase = new stdClass();
			$kase->case_uuid = $case_uuid;
			$kase->uuid = $case_uuid;
			$kase->case_id = $case_id;
			$kase->id = $case_id;
			$kase->case_name = $case_name;
			$kase->name = $case_name;
			$kase->case_number = $adj_number;
			$kase->file_number = "";
			$kase->worker = "";
			$kase->attorney = "";
			$kase->customer_id = $customer_id;
		} else {
			if ($kase->case_name != "") {
		...
		if (!is_object($kase)) {
			die("courtcalendar_id:" . $courtcalendar_id);
		}
...
function recordActivity($return, $customer_id, $operation, $activity, $case_uuid, $track_id, $category = "", $billing_time = 0) {
	try {
		...
		die("error recordActivity");
	}
}

$db_transfer = "court_calendar";
...
try {
	...
	//cer.eams_ref_number, cer.firm_name, 
	$sql = "SELECT cus.customer_id cus_id, cus.data_source, ccc.`courtcalendar_id`, ccc.`office`, ccc.`judge_name`, ccc.`worker_name`, ccc.`legacy_case_number`, ccc.`case_number`, ccc.`hearing_type`, ccc.`applicant_law_firm`, ccc.`defense_law_firm`, ccc.`hearing_time`, ccc.`hearing_location`
	...
	#AND worker_name = 'VASQUEZ, RAMIRO'
	AND ccc.import_date = '" . $import->import_date . "'";
	
	//ORDER BY cer.firm_name, ccc.hearing_time ASC
	
	//die($sql);
	echo "start " . date("g:i:sA") . "\r\n";
	$calendars = DB::select($sql);
	//print_r($calendars);
	foreach($calendars as $calendar) {
		...
		$sql = "SELECT ccc.*, 
		ccase.case_id, ccase.case_uuid, IF(ccase.case_number='', ccase.file_number, ccase.case_number) ikase_case_number
		FROM ikase.cse_eams_reps cer
		INNER JOIN ikase.cse_courtcalendar ccc
		ON cer.firm_name = ccc.applicant_law_firm
		INNER JOIN ikase.cse_customer cus
		...
		*/
		...
		//get the case worker
		if ($blnCaseExists) {
			if ($entry->case_number == "" && $entry->legacy_case_number!="") {
				$entry->case_number = $entry->legacy_case_number;
			}
			$kase = getKaseInfo($entry->case_id, $entry->worker_name, $entry->case_number, $calendar->courtcalendar_id, $customer_id, $db_name . ".");
		} else {
			if ($calendar->case_number == "" && $calendar->legacy_case_number!="") {
				$calendar->case_number = $calendar->legacy_case_number;
			}
			$kase = getKaseInfo(-1, $calendar->worker_name, $calendar->case_number, $calendar->courtcalendar_id, $customer_id, $db_name . ".");
		}
		//print_r($kase);
		//continue;
		$user_id = "";
		...
		if (is_object($entry)) {
			...
			//die($sql);
			$stmt = DB::run($sql);
			$case_event = $stmt->fetchObject();
		}
		$transfer_status = "pending";
		if (is_object($case_event)) {
			$transfer_status = "in";
		}
		//print_r($case_event);
		//get the case id for this case_uuid
		$sql = "SELECT case_id
		FROM court_calendar.cse_case
		WHERE case_uuid = '" . $kase->uuid . "'";
		$stmt = DB::run($sql);
		$courtcase = $stmt->fetchObject();
		
		$court_case_id = $kase->id;
		if (is_object($courtcase)) {
			$court_case_id = $courtcase->case_id;
		}
		//enter a new event if there is no event for this case in court_calendar
		$sql = "SELECT eve.*
		FROM " . $db_transfer . ".cse_event eve
		INNER JOIN " . $db_transfer . ".cse_case_event cce
		ON eve.event_uuid = cce.event_uuid
		INNER JOIN " . $db_transfer . ".cse_case ccase
		ON cce.case_uuid = ccase.case_uuid
		WHERE eve.event_dateandtime = '" . $calendar->hearing_time . "'
		AND ccase.case_id = '" . $court_case_id . "'
		AND eve.customer_id = '" . $customer_id . "'
		AND eve.event_type = '" . $calendar->hearing_type . "'";
		
		$stmt = DB::run($sql);
		$transfer_event = $stmt->fetchObject();
		//echo $sql . "\r\n";
		//die(print_r($transfer_event));
		
		if (!is_object($transfer_event)) {
			//put all of the events into court_calendar databse
			//indicate if already in
			//insert a new event
			$sql = "INSERT INTO " . $db_transfer . ".cse_event (`event_uuid`, `event_date`, `event_dateandtime`, `event_type`, `judge`, `full_address`, `customer_id`, `event_description`, `event_title`)";
			$event_uuid = uniqid("EV");
			$sql .= "
			VALUES ('" . $event_uuid . "', '" . date("Y-m-d", strtotime($calendar->hearing_time)) . "', '" . $calendar->hearing_time . "', '" . $calendar->hearing_type . "', '" . $calendar->judge_name . "', '" . $calendar->hearing_location . "', '" . $customer_id . "', '" . $calendar->hearing_type . "\r\n\r\nAdded via automatic Court Calendar Update on " . date("m/d/y") . "', '" . $calendar->hearing_type . "')";
			//echo $sql . "\r\n";
			//die();
			...
			$last_updated_date = date("Y-m-d H:i:s");
			$sql = "INSERT INTO " . $db_transfer . ".cse_case_event 
			(`case_event_uuid`, `case_uuid`, `event_uuid`, `attribute`, `transfer_status`, `last_updated_date`, `last_update_user`, `deleted`, `customer_id`)
			VALUES ('" . $event_uuid . "', '" . $kase->uuid . "', '" . $event_uuid . "', '" . $attribute . "', '" . $transfer_status . "', '" . $last_updated_date . "', 'system', 'N', '" . $customer_id . "')";
			//echo $sql . "\r\n";
			$stmt = DB::run($sql);
			...
			$stmt = $db->prepare($sql);
			$stmt->bindParam("event_uuid", $event_uuid);
			$stmt->bindParam("case_uuid", $kase->uuid);
			$stmt->bindParam("customer_id", $customer_id);
			$stmt->bindParam("courtcalendar_id", $calendar->courtcalendar_id);
			$stmt->execute();
		}
	...
	fwrite($fp, 'transfer csv  @ ' . date('m/d/y H:i:s') . chr(10));
	fclose($fp); 
	*/
	//launch the matrix script from here
	$params = array();
	$url = "https://www.matrixdocuments.com/dis/pws/courtcalendar_ping.php";
	curl_post_async($url, $params);
	
	die("done at " . date("m/d/Y g:i:sA"));
} catch(PDOException $e) {
	echo "error\r\n";
	echo $sql . "\r\n";
	echo '{"error":{"text":'. $e->getMessage() .'}}'; 
