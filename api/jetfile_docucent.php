...
$app->group('', function (RouteCollectorProxy $app) {
	$app->group('/jetfile', function (RouteCollectorProxy $app) {
		$app->get('/adj/check/{adj_number}', 'getADJCount');
		$app->get('/app/status/{filing_id}', 'checkFiling');
		$app->get('/fetch/{injury_id}', 'fetchDOIJetfile');

		//from ikase
		$app->post('/getpdf', 'getPDF');
		$app->post('/requestpdf', 'requestPDF');
		$app->post('/requestcreatepdf', 'requestCreatePDF');

		$app->post('/app_packet', 'createAppPacket');

		$app->group('/save', function (RouteCollectorProxy $app) {
			$app->post('/app', 'saveApp');
			$app->post('/dor', 'saveDOR');
			$app->post('/dore', 'saveDORE');
			$app->post('/lien', 'saveLien');
			$app->post('/unstructured', 'saveUnstructured');
		});

		$app->group('/check', function (RouteCollectorProxy $app) {
			$app->post('/applicant', 'checkApplicant');
			$app->post('/dor', 'checkDOR');
			$app->post('/dore', 'checkDORE');
		});

		$app->post('/send', 'sendAPP');
		$app->post('/resend', 'resendAPP');
		$app->post('/senddor', 'sendDOR');
		$app->post('/senddore', 'sendDORE');
		$app->post('/sendlien', 'sendLien');
		$app->post('/sendunstruc', 'sendUnstruc');

		$app->post('/updatecase', 'updateCase');
		$app->post('/app/filingid', 'updateAppFilingId');
		$app->post('/updateadj', 'updateCaseADJ');
		$app->post('/updatedor', 'updateDOR');
		$app->post('/dor/filingid', 'updateDORFilingId');
		$app->post('/updatedore', 'updateDORE');
		$app->post('/updatelien', 'updateJetLien');
		$app->post('/updateunstruc', 'updateUnstruc');
		$app->post('/unstruc/filingid', 'updateUnstrucFilingId');

		$app->post('/file', 'appFile');
		$app->post('/updateid', 'updateJetfileId');
		$app->post('/filedor', 'dorFile');
		$app->post('/filedore', 'doreFile');
		$app->post('/fileunstruc', 'unstrucFile');
	});
	
	$app->group('/jetfiles', function (RouteCollectorProxy $app) {
		$app->get('', 'fetchJetfiles');
		$app->get('/all', 'fetchAllJetfiles');
		$app->get('/errors', 'fetchErrorJetfiles');
		$app->get('/recent', 'fetchRecentJetfiles');
		$app->get('/search/{search_term}', 'searchJetfiles');
	});
})->add(Api\Middleware\Authorize::class);

$app->group('/jetfile', function (RouteCollectorProxy $app) {
	$app->get('/app/checkstatus', 'verifyPendingAPP');
	$app->get('/monitor', 'monitorFilings');
});

//from cajetfile
$app->post('/acceptpdf', 'acceptPDF');
...
function acceptPDF() {
	...
	$filename_output = UPLOADS_PATH . $cus_id . DC . $case_id . "\\jetfiler\\" . $filename_output;
