<?php
$app->group('/bodyparts', function (\Slim\Routing\RouteCollectorProxy $app) {
	$app->get('/{case_id}/{injury_id}', 'getBodyparts');
	$app->post('/add', 'setBodyparts');
		...
		if (isset($_POST["bodypart"  .$int])) {
			if ($_POST["bodypart"  .$int]!="") {
				...
				try {
                    ...
                    echo json_encode(array("error" => array("text" =>$e->getMessage(), "sql" =>$sql)));
				}
			}
		}
	}
