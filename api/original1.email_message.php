...
*/
	...
	//Solulab code change START 13-08-2019
	Function SendMail($to, $subject, $body, $headers, $return_path,$cc="",$bcc="",$from=""){
		if(!function_exists("mail"))
			return($this->OutputError("the mail() function is not available in this PHP installation"));
	
		if(strlen($return_path))
		{
					die("sending mail:" . $return_path);
			if(!defined("PHP_OS"))
			...
			// //$headers = str_replace("multipart/mixed; boundary", "multipart/related; boundary", $headers);
			// echo $headers . "\r\n" . "\r\n";
			// //echo $body;
			// //$to = str_replace("<", "", $to);
			// //$to = str_replace(">", "", $to);
			// $to = str_replace("support@ikase.org", "mccraney66@gmail.com", $to);
			...
			//echo $headers . "\r\n" . "\r\n";
			//echo $body;
			//Value change operation on TO,CC,BCC
			$to = str_replace("<", "", $to);
			$to = str_replace(">", "", $to);
			$cc = str_replace("<", "", $cc);
			$cc = str_replace(">", "", $cc);
			$bcc = str_replace("<", "", $bcc);
			$bcc = str_replace(">", "", $bcc);
			// $headers = "From: donotreply@ikase.website" . "\r\n";
			// if(isset($cc) && $cc!=""){
			// 	$headers .= "Cc: $cc \r\n";
			// }
			// if(isset($bcc) && $bcc!=""){
			...
			if(isset($from) && $from!=""){
				$start=strpos($from,'<')+1;
				$end=strpos($from,'>');
				$fromname=substr($from,0,$start-1);
			...
			// $subject = "My subject";
			// $txt = "Hello world!";	
			// $headers = "From: admin@ikase.website" . "\r\n";
			// $success = mail($to,$subject,$txt,$headers);
			//die("succ");
			//echo $to . "<br /><br />" . $subject . "<br /><br />" . $body . "<br /><br />" . $headers . "<br />";
			
		//	print_r($body);			
			$link_pos=strpos($body,"http");
			$cell_pos=strpos($body,"Cell:");
			$body=substr_replace( $body,"<br>",$link_pos, 0 );
			$body=substr_replace( $body,"<br>",($cell_pos+4), 0 );
		//	print_r($body);
		//	die();
			...
			try {
				...
				$mail->Username   = 'ikase.website';                     // SMTP username
				...
				if(isset($from) && $from!=""){
					...
					$mail->setFrom('donotreply@ikase.website', 'Ikase');
				}
				...
				foreach($this->parts as $files)
				{
					if(isset($files['FILENAME']) && $files['FILENAME'])
					{
	...
	//Solulab code change END 13-08-2019
	...
	Function EndSendingMessage()
	{
		...
		foreach($this->parts as $files)
		{
			if(isset($files['Content-Type']) && $files['Content-Type']=="text/html")
			{
				$bodydata=$files['DATA'];
			}
		}
		// print_r($bodydata);
		// die();
		if(strcmp($error=$this->SendMail(strlen($has["to"]) ? $has["to"] : (IsSet($has["cc"]) ? "" : "undisclosed-recipients: ;"), $has["subject"], $bodydata, $headers_text, IsSet($has["return-path"]) ? $has["return-path"] : "",IsSet($has["cc"]) ? $has["cc"] : "",IsSet($has["bcc"]) ? $has["bcc"] : "",IsSet($has["from"]) ? $has["from"] : ""),""))
		...
		switch($type)
		{
			...
			case "message":
				if(IsSet($this->parts[$part]["FILENAME"])){
					$filename = $this->parts[$part]["FILENAME"];
					$filename = urldecode($filename);
					
					$size=@filesize($filename);
					if(!($file=@fopen($filename,"rb")))
						return($this->OutputPHPError("could not open part file ".$filename, $php_errormsg));
					while(!feof($file))
					{
					...
					if(GetType($size)=="integer"
					&& strlen($body)!=$size)
						return($this->OutputError("the length of the file that was read does not match the size of the part file ".$filename." due to possible data corruption"));
				}
				else
				{
			...
			case "multipart":
				switch($sub_type)
				{
					...
					case "parallel":
						...
						$b = $this->line_break;
						$lb = strlen($b);
						//Solulab Change 21-08-2019
						for($multipart=1;$multipart<2;$multipart++)
						{
							if(strlen($body) >= $lb
...
*/
	Function SetMultipleEncodedEmailHeader($header,$addresses, $encoding_charset = '')
	{
		...
		for($value="";!$end;)
		{
			...
			//die($value);
			$value .= chr(34) . $address . chr(34) .' <'.$address.'>';
			Next($addresses);
			$end=(GetType($address=Key($addresses))!="string");
		}
...
*/
	...
	Function GetHeadersAndBody(&$headers, &$body)
	{
		...
		{
			//Solulab comment blank body come after 3511
			//Solulab data change after this
		...
		//Solulab comment Wrong data Come after 3511
		return("");
	}
...
*/
	Function Send()
	{
		...
		//echo "head:" . strlen($error=$this->SendMessageHeaders($headers)) . "<br />";
		//die();
		//die($this->SendMessageBody($body));
		//echo "body:" . strlen($error=$this->SendMessageBody($body)) . "<br />";
		//
		//die();
		if(strlen($error=$this->SendMessageHeaders($headers))==0 && strlen($error=$this->SendMessageBody($body))==0) {
			$error = $this->EndSendingMessage();
		} else {
			die("error sending:" . $error);
		}
