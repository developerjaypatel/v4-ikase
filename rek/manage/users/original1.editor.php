...
if ($user_id!="") {
	...
	`user_logon`, `user_first_name`, `user_last_name`, `user_type`
	FROM rek_user 
	WHERE 1";
...
if ($numbs>0) {
	...
	$user_type = mysql_result($result, $int, "user_type");
...
for ($int=0;$int<$numbs;$int++) {
	$job_id = mysql_result($result, $int, "job_id");
	$job_uuid = mysql_result($result, $int, "job_uuid");
	$the_job = mysql_result($result, $int, "job");
	$selected = "";
	if ($job==$the_job) { 
		$selected = " selected"; 
	}
	$option = "<option value='" . $the_job . "'" . $selected . ">" . $the_job . "</option>";
	$job_options .= "" . $option;
}
*/
?>
