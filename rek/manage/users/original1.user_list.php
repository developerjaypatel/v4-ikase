...
//die($cus_id);
$query = "SELECT  `user_id`,`user_name`, `user_email`, `nickname`, `pwd`, `level`, `job`, `user_logon`, `deleted`
FROM `rek`.rek_user 
...
for ($int=0;$int<$numbs;$int++) {
	...
	if ($nickname=="" && $user_id !="") {
		...
		$queryupdate = "UPDATE rek_user SET `nickname` = '" . $nickname . "' WHERE user_id = " . $user_id;
