...
$query = "";
if ($cus_id=="" || $cus_id==-1) {
	$query = "INSERT INTO `rek`.rek_customer (`eams_no`, `parent_customer_id`, `cus_name`, `cus_name_first`, `cus_name_middle`, `cus_name_last`, `cus_street`, `cus_city`, `cus_county`, `cus_state`, `cus_zip`, `cus_email`, `cus_phone`, `cus_fax`, `cus_type`, `cus_ip`, `pwd`, `admin_client`,`data_source`,`data_path`,`permissions`,`inhouse_id`, `ddl_venue`,`office_manager_first`, `office_manager_last`, `office_manager_middle`, `office_manager_phone`, `office_manager_email`, `cus_fedtax_id`, `cus_uan`, `cus_barnumber`, `user_rate`, `corporation_rate`)
	VALUES ('" . addslashes($eams_no) . "', '" . addslashes($parent_cus_id) . "' , '" . addslashes($cus_name) . "', '" . addslashes($cus_name_first) . "', '" . addslashes($cus_name_middle) . "', '" . addslashes($cus_name_last) . "', '" . addslashes($cus_street) . "', '" . addslashes($cus_city) . "', '" . addslashes($cus_county) . "', '" . addslashes($cus_state) . "', '" . addslashes($cus_zip) . "', '" . addslashes($cus_email) . "', '" . addslashes($cus_phone) . "', '" . addslashes($cus_fax) . "', '" . addslashes($cus_type) . "', '" . addslashes($cus_ip) . "', '" . addslashes($pwd) . "', '" . addslashes($admin_client) . "','" . $data_source . "','" . $data_path . "','" . $permissions . "','" . $inhouse_id . "', '" . $letter_office_code . "','" . addslashes($office_manager_first) . "','" . addslashes($office_manager_last) . "','" . addslashes($office_manager_middle) . "','" . addslashes($office_manager_phone) . "','" . addslashes($office_manager_email) . "','" . addslashes($cus_fedtax_id) . "','" . addslashes($cus_uan) . "','" . addslashes($cus_barnumber) . "','" . addslashes($user_rate) . "','" . addslashes($corporation_rate) . "')";
	$result = mysql_query($query, $r_link) or die("unable to insert customer<br>" . mysql_error());
	$cus_id = mysql_insert_id($r_link);
	//now update the uuid
	$query = "UPDATE rek.rek_customer
	SET customer_uuid = customer_id
	WHERE customer_id = " . $cus_id;
	$result = mysql_query($query, $r_link) or die("unable to UPDATE customer<br>" . mysql_error());
	...
	$query = "INSERT INTO `rek`.`rek_user` (`user_uuid`, `customer_id`, `user_name`,`user_email`, `pwd`, `level`, `job`, `day_start`, `day_end`, `days_of_week`, `dow_times`, `user_logon`, `user_first_name`, `user_last_name`, `user_type`)
	...
	$result = mysql_query($query, $r_link) or die("unable to insert user<br>" . $query . "<br />" . mysql_error());
	...
	//die($query);
	$result = mysql_query($query, $r_link) or die("unable to update customer<br>" . $query . "<br />" . mysql_error());
}
...
$query = "UPDATE 
rek.rek_customer 
SET parent_customer_id = " . $parent_cus_id . 
" WHERE customer_id = " . $cus_id;
$result = mysql_query($query, $r_link) or die("unable to update customer parent<br>" . $query);
