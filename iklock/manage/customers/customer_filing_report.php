...
//echo $querylate;
$resultlate = DB::runOrDie($querylate);

$current_cus_id = -1;
$arrRows = array();

$first = true;
while ($row = $resultlate->fetch()) {
	$the_cus_id = $row->cus_id;
	...
	$serve_check_amount = $row->serve_check_amount;
	$custodian_check_amount = $row->custodian_check_amount;
	...
	$the_case_id = $row->case_id;
	$arrCases[$the_cus_id]++;
	$the_submitted_date = $row->filing_date;
	$invoice_total = $row->invoice_total;
	$serve_check_amount = $row->serve_check_amount;
	$custodian_check_amount = $row->custodian_check_amount;
