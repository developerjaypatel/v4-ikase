...
$query_notes = "SELECT cn . * FROM ikase.cse_notes cn WHERE cn.customer_id = 1033 AND notes_id =" . $note_id;
$row = DB::runOrDie($query_notes)->fetch();
$subject = $row->subject;
$note = $row->note;
?>
...
<input type="hidden" value="<?php echo $suid; ?>" name="suid" id="suid" />
    ...
    <table width="700" border="0" align="center" cellpadding="2" cellspacing="0">
	  ...
	  <tr>
		...
		<input type="text" name="subject" id="subject" style="width:99%" value="<?php echo $subject; ?>" /></td>
	  </tr>
	  <tr>
	  ...
	  <tr>
		...
		<td colspan="2" valign="top">
			<textarea name="note" id="note" style="width:99%" rows="5"><?php echo $note; ?></textarea>
		</td>
	  </tr>
	  <tr>
