...
$my_business_address = new address();
$my_phone = new comm();
...
if ($user_id > -1) {
	...
	//echo $querypayrate . "<BR>";
	//$resultupdate = MYSQL_QUERY($queryupdate, $link) or die("Unable to get user<br>$queryupdate<br>" . mysql_error());
	...
	where user_logon = '$user_logon'
	order by user_id desc";
	//$resultcommission = MYSQL_QUERY($querycommission, $link) or die("Unable to get user<span style='font-weight:bold'>$querycommission");
	//$numbercommission = mysql_Numrows($resultcommission);
	try {
		$sql = $querypayrate;
		$db = getConnection();
		$stmt = $db->prepare($sql);  
	...
	//if ($numberupdate>0) {
		...
		//$numberevents = mysql_numrows($resultevents);
	...
	//	echo "Phone: " . $phone_uuid . "<BR>";
		...
		//echo $query_days . "<Br>";
		//$result_days = mysql_query($query_days, $link) or die("unable to get timeoff info");
		$days_available = 0;
	...
	//	echo $queryrates;
		//$resultrates = MYSQL_QUERY($queryrates, $link) or die($queryrates . "<br>" . mysql_error());
		//$numberrates = mysql_Numrows($resultrates);
