<?php
$app->group('/employee', function (\Slim\Routing\RouteCollectorProxy $app) {
	$app->get('/notes/{user_id}', 'getEmployeeNotes');

	$app->post('/save', 'saveEmployee');
	$app->post('/savecontact', 'saveContact');
	$app->post('/saveemployment', 'saveEmployment');
	$app->post('/savenote', 'saveEmployeeNote');
	$app->post('/savetaxfederal', 'saveTaxFederal');
	$app->post('/savetaxstate', 'saveTaxState');
	$app->post('/savecontractor', 'saveContractor');
...
function getEmployeeNotes($user_id) {
    echo json_encode((new notes())->fetch_employee_notes($user_id));
}
