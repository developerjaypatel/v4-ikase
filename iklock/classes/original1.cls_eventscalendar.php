...
//		$event_table = "<tr><td align=left colspan='2'>&nbsp;</td></tr>";
		...
		$numberdayoff = mysql_numrows($resultdayoff);
		$arrUsers = array();
		$arrApproved = array();
		$arrReviewed = array();
		for ($intD=0;$intD<$numberdayoff;$intD++) {
			$the_event_uuid = mysql_result($resultdayoff, $intD, "events_uuid");
			$arrEventsID[] = $the_event_uuid;
			$the_user_logon = mysql_result($resultdayoff, $intD, "user_logon");
			$arrUsers[] = $the_user_logon;
			$arrUserUUID[] = mysql_result($resultdayoff, $intD, "user_uuid");
			...
			$approved_dates  = mysql_result($resultdayoff, $intD, "approved_dates");
			$type  = mysql_result($resultdayoff, $intD, "attribute");
			$description  = mysql_result($resultdayoff, $intD, "description");
			$arrApprovedDates = explode("|", $approved_dates);
			$active = mysql_result($resultdayoff, $intD, "active");
			$verified = mysql_result($resultdayoff, $intD, "verified");
			$time_stamp = mysql_result($resultdayoff, $intD, "time_stamp");
			if ($active == "Y") {
		...
		$numbertimeoff = mysql_numrows($resulttimeoff);
/*
		for ($intD=0;$intD<$numbertimeoff;$intD++) {
			$arrNotesID[] = mysql_result($resulttimeoff, $intD, "notes_id");
			$arrTimeUsers[] = mysql_result($resulttimeoff, $intD, "user_logon");
			$arrTimeApproved[] = mysql_result($resulttimeoff, $intD, "active");
			//just for that user, if we need it later
			$arrTimes[$user_logon]++;
		}
*/
    ...
    */
	...
	function getDaysBlocked($month, $year) {
		...
		for ($intX=0;$intX<$numbers;$intX++) {
			$blocked_date = mysql_result($result, $intX, "blocked_date");
