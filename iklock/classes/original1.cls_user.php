...
class systemuser
{
	...
	function make_selectoptions($group, $sortby = "", $attribute = "") {
		...
		$numberdrop = mysql_numrows($resultset);
...
//				echo $category . "<br>";
	...
	function getAssignments($id="", $status = "") {
		...
		if ($numberupdate>0) {
			...
			$numberass = mysql_Numrows($resultass);
...
//					echo $status_number . "<BR>";
					...
					if ($singleUser) {
						...
						$numbnote = mysql_numrows($resultnote);
						if ($numbnote > 0) {
...
//					echo $currentday ."!=" . $dateandtime . "=" . strlen($day_cell) . "<BR>";
	...
	function getAssignmentsVertical($id="", $status = "", $thedate = "") {
		...
		if ($numberupdate>0) {
			...
			$numberords = mysql_Numrows($resultords);
		...
		//get the locations
			...
			if ($numberass >0) {
				for ($intA=0;$intA<$numberass;$intA++) {
					$location_id = mysql_result($resultass, $intA, "location_id");
					$dateandtime = mysql_result($resultass, $intA, "dateandtime");
					$assignment_type = mysql_result($resultass, $intA, "assignment_type");
					$status_number = mysql_result($resultass, $intA, "status_number");
					$blnShowRow = true;
