...
//		$filter = addslashes($filter);
		...
		try {
			...
			$stmt = $db->prepare($sql);  
			$stmt->execute();
			$resultdepartment = $stmt->fetchAll(PDO::FETCH_OBJ);
	...
	function id($id) {
		...
		if ($this->uuid=="") {
			...
			$numberdepartment = mysql_Numrows($resultdepartment);
	...
	function fetch($department_id = "") {
		...
		//$resultdepartment = MYSQL_QUERY($querydepartment,$this->datalink)	or die ("Unable to get the department info<br>$querydepartment<br>" . mysql_error());
		//$numberdepartment = mysql_Numrows($resultdepartment);
