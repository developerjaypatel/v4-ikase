...
class Calendar
{
    ...
    */
    function getDateLink($day, $month, $year) {
		...
		$numbernews = mysql_numrows($resultnews);
		//die($querynews);
		//default
		$return = "&nbsp;";
		if ($numbernews > 0) {
			...
			$destination = "edit";
			$return = "<a href='" . $destination . ".php?team_id=" . $this->team_id . "&id=" . $news_id . "&uuid=" . $news_uuid . "&cat=" . $this->category_name . "&edit=y'>" . $news . "</a>";
		}
		return $return;
    }
    	...
    	while ($d <= $daysInMonth)
    	{
    	    ...
    	    for ($i = 0; $i < 7; $i++) {
    	        if ($d > 0 && $d <= $daysInMonth) {
					...
					$s .= "<a href='new.php?calendarDate=" . $today . "&edit=y&cat=calendar&team_id=" . $this->team_id . "' title='Click here for new calendar item'>new</a>&nbsp;&nbsp;<span class=\"" . $theclass . "\">$d</span>";
					if ($link != "&nbsp;") {
						$s .= "<br>" . $link;
					}					
    	        } else {
