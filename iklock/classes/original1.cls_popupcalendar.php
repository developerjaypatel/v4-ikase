...
class Calendar
{
    ...
    */
    function getCalendarLink($month, $year)
    {
		...
		return "$s?month=$month&year=$year&datefield=" . $this->datefield;
    }
    ...
    */
    function getDateLink($day, $month, $year)
    {
		...
		*/
		...
		$yesterday = date("Y-m-d", mktime(0, 0, 0, $month  , $day-1, $year));
		$tomorrow = date("Y-m-d", mktime(0, 0, 0, $month  , $day+1, $year));
		if ($this->standard_report=="true") {
			//count the calls for the user
			if ($this->type == "billing") {
				$callbacks_today = $this->billing_callbacks_count($yesterday, $tomorrow, $this->username);
			} else {
				//$callbacks_today = $this->callbacks_count($yesterday, $tomorrow, $this->username);
				$callbacks_today = $this->callbacks_employee_count($yesterday, $tomorrow, $this->username);
			}
			//return something
			if ($callbacks_today=="0") {
				return "";
			} else {
				return $callbacks_today;
			}
		} else {
			return "&nbsp;";
		}
...
//			
	...
	function callbacks_count($start_date, $end_date, $USERNAME) {
		$statusquery = "SELECT count(notes_status.location_id) location_count
		FROM `notes_status` notes_status
		inner join `notes` notes
		on notes_status.max_notes_id = notes.notes_id
		inner join location
		on notes_status.location_id = location.location_id
		inner join `orders` ord 
		on location.order_id = ord.order_id
		where ifnull(location.cancelled,'N') <> 'Y' and `max_notes_id` = `max_callback_id`";
		if ($start_date!="") { 
			$statusquery .= " AND `actual_callback_date` > '" . $start_date . "'";
		}
		$statusquery .= " AND `actual_callback_date` < '" . $end_date . "'";
	...
	function billing_callbacks_count($start_date, $end_date, $USERNAME) {
		...
		where `max_billing_notes_id` = `max_callback_id`";
		if ($start_date!="") { 
			$statusquery .= " AND `actual_callback_date` > '" . $start_date . "'";
		}
		$statusquery .= " AND `actual_callback_date` < '" . $end_date . "'";
    ...
    */
    function getMonthHTML($m, $y, $showYear = 1)
    {
    	...
    	$s .= "<tr>\n";
    	$s .= "<td align=\"center\" valign=\"top\">" . (($prevMonth == "") ? "&nbsp;" : "<a href=\"$prevMonth\" class=\"thenumber\">&lt;&lt;</a>")  . "</td>\n";
    	$s .= "<td align=\"center\" valign=\"top\" class=\"calendarHeader\" colspan=\"5\">$header</td>\n"; 
    	$s .= "<td align=\"center\" valign=\"top\">" . (($nextMonth == "") ? "&nbsp;" : "<a href=\"$nextMonth\" class=\"thenumber\">&gt;&gt;</a>")  . "</td>\n";
    	$s .= "</tr>\n";
    	...
    	while ($d <= $daysInMonth)
    	{
    	    ...
    	    for ($i = 0; $i < 7; $i++)
    	    {
    	        ...
    	        if ($d > 0 && $d <= $daysInMonth)
    	        {
					...
					if ($link == "" || $link == "&nbsp;") {
						$s .= "<a href=\"javascript:closeWindow('$today')\" class=\"" . $theclass . "\">$d</a>";
					} else {
						$s .= "<a href=\"javascript:closeWindow('$today')\" class=\"" . $theclass . "\">$d</a><br><i>$link</i>";
					}
    	        }
    	        else
    	        {
    ...
    */
	...
	function reset_link() {
		mysql_close($this->datalink);
		$this->datalink = mysql_connect($this->data_hostname, $this->data_username, $this->user_pwd) OR die("Unable to connect to database");       
/* Select the database name to be used or else print error message if unsuccessful*/
		mysql_select_db($this->data_db, $this->datalink) or die( "Unable to select database"); 
	}
    /* 
