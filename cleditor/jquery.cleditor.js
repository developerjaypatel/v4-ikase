...
(function ($) {
    ...
    function refresh(editor) {
        ...
        // Create a new iframe
        var $frame = editor.$frame = $('<iframe frameborder="0" src="javascript:true;" id="cleditor_frame" />')
            .hide()
            .appendTo($main);
        ...
        doc.write(
            ...
            '<body class="iframe_bulk" style="' + options.bodyStyle + '"></body></html>'
        );
		//<script>setTimeout(function() { document.getElementById("iframe_bulk").focus(); }, 500)</script>
        doc.close();
        ...
        $doc.click(hidePopups)
			...
			.bind("paste", function() {
				...
				setTimeout(function() {
					...
					setTimeout(function() {
						var thebody = $(editor.doc.body).html();
						
						thebody = thebody.replaceAll('<p', '@@@@');
						thebody = thebody.replaceAll('</p>', '~~~');
						
						thebody = thebody.replaceAll('<span', '#@#@#@#@');
						thebody = thebody.replaceAll('</span>', '######');
						
						//thebody = thebody.replaceTout('<br style="box-sizing: border-box; color: rgb(255, 255, 255); font-family: arial; font-size: 16px; background-color: rgb(25, 25, 25);">', '<br>');
						thebody = thebody.replaceTout('<br style', '~@~@~@$#%#$#');
						thebody = thebody.replaceTout('$#%#$#', '<br style');
						thebody = thebody.replaceTout('<br>', '~@~@~@');
						thebody = thebody.replaceTout('<br />', '~@~@~@');
						
						//divs
						thebody = thebody.replaceTout('<div>', 'd@d@d@');
						thebody = thebody.replaceTout('</div>', 'e@e@e@');
						
						//remove all html not
						thebody = removeHtml(thebody);
						
						thebody = thebody.replaceTout('~@~@~@', '<br>');
						thebody = thebody.replaceTout('@@@@', '<p');
						thebody = thebody.replaceTout('~~~', '</p>');
						
						thebody = thebody.replaceTout('#@#@#@#@', '<span');
						thebody = thebody.replaceTout('######', '</span>');
						thebody = thebody.replaceTout('color: rgb(255, 255, 255)', '');
						thebody = thebody.replaceTout('color: rgb(34, 34, 34)', '');
						thebody = thebody.replaceTout('color: rgb(25, 25, 25)', '');
						thebody = thebody.replaceTout('background-color: rgb(25, 25, 25)', '');	//background-color: rgb(255, 255, 255)
						
						//divs
						thebody = thebody.replaceTout('d@d@d@', '<div>');
						thebody = thebody.replaceTout('e@e@e@', '</div>');
						
						$(editor.doc.body).html(thebody);
            ...
            .bind("keyup mouseup", function () {
				/*
				var editor_content = $(editor.doc.body).html();
				
				if (editor_content.length==1) {
					$(editor.doc.body).html(editor_content.toUpperCase())
				}
				*/
                refreshButtons(editor);
                updateTextArea(editor, true);
        ...
        $(function () {
            ...
            // Resize the iframe
            //hgt = (/%/.test("" + options.height) ? $main.height() : parseInt(options.height, 5)) - hgt  + 150;
			if (options.height!="") {
				hgt = options.height;
			} else {
				hgt = $main.height ;
    ...
    function updateTextArea(editor, checkForChange) {
        ...
        if (code !== $area.val()) {
			...
			if (html.length==1) {			
				setTimeout(function() {
					$(editor.doc.body).html(html.capitalizeWords());
					code = html.capitalizeWords();
					editor.$frame[0].contentWindow.focus();
				}, 100);
			}
			*/
