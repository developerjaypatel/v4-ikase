...
$page_title = "Contact Us $application";
?>
...
<!-- Main jumbotron for a primary marketing message or call to action -->
    <div class="jumbotron" style="border-bottom: 1px solid black;">
      <div class="container">
        <h1>Contact <?= $application; ?> Support</h1>
        <p><?= $application; ?> development relies on the constant and pointed feedback of our customers.  Bugs are fixed immediately upon receipt, and custom requests are processed as soon as possible.  We appreciate your feedback and guidance as we build the best system for legal professionals.</p>
      </div>
    </div>
    ...
    <div class="container"> 
        ...
        <?php } else { ?>    
        <div id="form_holder">
			<form action="<?= $form_action_url; ?>" method="post" enctype="multipart/form-data" id="main_form">
            	<div>
                ...
                <div>
                	<input type="checkbox" name="existing" value="Y"> 
                    <?php
                    if($application == "iKase")
                    { 
                    ?>
                    I am an <?= $application ?> user
                    <?php
                    }
                    else
                    {
                    ?>
                    I am a <?= $application ?> user
                    <?php
                    }
                    ?>
                </div>
                ...
                <div onBlur="releaseSave()">
                	<div class="g-recaptcha" data-sitekey="<?= $application_sitekey; ?>" data-callback="enableBtn"></div>
                </div>
                <div>
