...
function insertReminders() {
    ...
    for ($i=0; $i < $total_count; $i++) { 
        ...
        try {
            $db = getConnection();
            $stmt = $db->prepare($get_phone_query);
            $stmt->execute();
...
function updateReminders() {
    ...
    try {
        $db = getConnection();
        $stmt = $db->prepare($get_event_uuid);
        $stmt->execute();
        $event_object = $stmt->fetchAll(PDO::FETCH_OBJ);
    ...
    for ($i=0; $i < $total_count; $i++) { 
        ...
        try{           
            ...
            //message
            $db = getConnection();
            $stmt = $db->prepare($sql_first);
            $stmt->execute();
            $message_id_first = $db->lastInsertId();
            $db = null; $stmt = null;
            //mesage_user
            $db = getConnection();
            $stmt = $db->prepare($str_query_first);
            $stmt->execute();
            $db = null; $stmt = null;
            //reminder
            $db = getConnection();
            $stmt = $db->prepare($strSQL_first);
            $stmt->execute();
            ...
            //message
            $db = getConnection();
            $stmt = $db->prepare($sql_second);
            $stmt->execute();
            $message_id_second = $db->lastInsertId();
            $db = null; $stmt = null;
            //mesage_user
            $db = getConnection();
            $stmt = $db->prepare($str_query_second);
            $stmt->execute();
            $db = null; $stmt = null;
            //reminder
            $db = getConnection();
            $stmt = $db->prepare($strSQL_second);
            $stmt->execute();
            $reminder_id_second = $db->lastInsertId();
            $db = null; $stmt = null;
            //reminder_message
            $db = getConnection();
            $stmt = $db->prepare($query_second);
            $stmt->execute();
            $db = null; $stmt = null;
            //event_reminder
            $db = getConnection();
            $stmt = $db->prepare($str_SQL_second);
            $stmt->execute();
            $db = null; $stmt = null; 
