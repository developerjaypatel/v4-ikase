...
if ($form_name!="bodyparts") {
	include_once("../api/manage_session.php");
	session_write_close();
...
<% kase_type = ""; %>
<?php } ?>
<?php $height_holder = "30px";
$height_holders = "0px";
 ?>
<div id="sub_category_holder<?php echo "_" . $form_name; ?>" class="<?php echo $form_name; ?>" style="text-align:left; padding-bottom:<?php echo $height_holder; ?>; height:<?php echo $height_holders; ?>">
	<span style="text-align:left;">
    	<span style="color:#FFFFFF; font-size:1.4em; font-weight:lighter; margin-left:10px;">
		<span id="panel_title" style="height:30px; border:0px solid black"><%	//kase_type.replace("_", " ").capitalizeWords() %>
        	<%  if (blnPiReady) { %>
       ...
       <div style="float:right;border:0px solid red; height:30px" id="<?php echo $form_name; ?>_buttons">
            ...
            &nbsp;&nbsp;&nbsp;
            <?php if ($form_name=="injury") { ?>
            	<div class="injury_links" style="display:inline-block; z-index:6234; margin-left:10px; margin-top:-10px; border:0px solid red;">
            		<a id='scrape_injury_<%=injury_id %>' class='scrape_injury white_text' style='font-size:0.9em;cursor:pointer; margin-right:-10px; display:inline-block' title='Click to import data from EAMS and update this injury'>eams update</a><br />
		            <a href='reports/demographics_sheet.php?case_id=<%=case_id %>&injury_id=<%=injury_id %>' style='font-size:0.9em;cursor:pointer; color:white; margin-right:-10px; display:inline-block' title='Click to create a Demographics sheet for this DOI only' target="_blank" id="injury_demographics_link">demographics</a>
            	</div>
            <?php } ?>  
            
            <?php if ($blnShowButtons) { ?>
           <span class="edit_row <?php echo $form_name; ?>" style="display:inline-block; z-index:6234; margin-left:25px; margin-top:-10px; height:35px; border:0px solid black">
           <button id="partie_edit" class="edit btn btn-transparent border-blue" style="border:0px solid; <?php if ($form_name=="injury") { ?>margin-top:-30px<?php } ?>; width:20px; display:"><i class="glyphicon glyphicon-edit" style="color:#0033FF">&nbsp;</i></button>
           </span>
           <span class="button_row <?php echo $form_name; ?> hidden" style="display:inline-block; margin-left:25px; margin-top:-10px">
               ...
               &nbsp;<button class="save btn btn-transparent border-green" style="width:20px; border:0px solid; <?php if ($form_name=="injury") { ?>margin-top:-30px<?php } ?>; height:30px"><i class="glyphicon glyphicon-saved" style="color:#00FF00">&nbsp;</i></button>
           </span>
           <?php } ?>
       </div>
    </span>   
