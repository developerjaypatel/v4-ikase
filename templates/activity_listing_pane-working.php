...
If No, you will be able to confirm transfer later">
	...
	<div class="glass_header">
        <button class="btn btn-primary btn-xs" id="send_documents_2" title="Click to Send checked Documents via Email/Interoffice"  style="visibility:hidden;" >Send Checked</button>

		<form id="activity_operation_form" method="POST" action="../merge_documents/activity.php">
		<input type="hidden" name="ids" id="ids"/>
		<input type="hidden" name="operation" id="operation"/>
		<input type="hidden" name="case_id" id="case_id" value="<%=case_id %>"/>
		<button type="button" class="btn btn-success" id="my_button_01">
			<i class="glyphicon glyphicon-envelope" style="color:white"></i>
			<span style="color:white">Email Docs</span>
		</button>
		<button type="button" class="btn btn-success" id="my_button_11">
			<i class="glyphicon glyphicon-align-justify" style="color:white"></i>
			<span style="color:white">Merge Docs</span>
		</button>
		<button type="button" class="btn btn-success" id="my_button_21">
			<i class="glyphicon glyphicon-download-alt" style="color:white"></i>
			<span style="color:white">Download as Zip</span>
		</button>
		</form>
    </div>
	<!-- NISHIT CODE ENDS -->
    ...
    <table id="activity_listing" class="tablesorter activity_listing" border="0" cellpadding="0" cellspacing="1">
        ...
        <tr class="activity_data_row activity_data_row_<%=activity.id %> activity_data_row_<%=activity.activity_uuid %> <% if (activity.billed) { %>billed_data_row<% } %> <% if (activity.billable) { %>billable_data_row<% } %> <% if (activity.billed || activity.billable) { %>invoice_data_row<% } %>">
            ...
            <% if(activity.activity_category=="Documents") { %>
			<input type="checkbox" class="check_thisone234" data-id="<%= activity.activity %>" data-doc-date="<%= activity.activity_date %>" data-doc-activity="<%= activity.activity_id %>" 
			data2="<%= activity.full_activity %>"/>
			<% } %>
            <a id="open_activity_<%=current_case_id %>_<%= activity.activity_uuid %>_<%= activity.activity_id %>" class="open_activity" style="; cursor:pointer; padding:2px">
			<i style="font-size:15px;color:#06f; cursor:pointer;" class="glyphicon glyphicon-edit" title="Click to Edit Activity"></i>
			</a>
            <% if(activity.ktags == null || activity.ktags == "") { %>
            No Ktags
            <% } else { %>
            <span title="Activity Ktags"><%= activity.ktags %></span>
            <% } %>
            </td>
          ...
          <td valign="top" width="1%" nowrap="nowrap">
          	...
          	<span class="activity_category activity_category_span" id="activity_category_<%=activity.activity_uuid %>" style="cursor:pointer" title="Click to Filter by this category"><%= activity.activity_category %>
            
              <% if(activity.activity_category=="email") {%>
                &nbsp; Receive
            ...
            <td valign="top">
            	<% if (customer_id == 1033) { %>
                    <% if(activity.activity_category=="Documents"){%>
                         <%=  activity.activity = activity.activity.replaceAll("target='_blank'","target='_blank' style='background:yellow;color:black'").replaceAll("</a>,","</a><br />")
                         %>
                        <%}else{%>
                            <%= activity.activity %>
                    <% }%>
...
<div id="download_file__popup" class="modal-popup">
  <div class="modal-content-popup">
	...
	<p class="modal-popup-note"></p>
	<p class="modal-popup-button"><input type="button" class="modal-button-popup btn btn-success" onclick='$("#activity_operation_form").submit();$("#download_file__popup").hide();'/></p>
  </div>
...
$("#my_button_01").click(function() {
	show_modal_popup('email');
});
$("#my_button_11").click(function() {
	show_modal_popup('merge');
});
$("#my_button_21").click(function() {
	show_modal_popup('zip');
});
$(".close-popup").click(function() {
	$("#download_file__popup").hide();
});
function show_modal_popup(type) {
	var message = "";
	$("#download_file__popup .modal-popup-note").html("");
	if(type == "merge") {
		message = "Please select documents to perform merge operation!";
		$("#download_file__popup .modal-title-popup").html("Selected Documents");
		$("#download_file__popup .modal-button-popup").val("Merge");
		$("#download_file__popup .modal-popup-note").html("<b>Note:</b> Only Docx and Pdf documents will be merge in to single PDF, rest other will be ignored.");
	} else if(type == "zip") {
		message = "Please select documents to download as zip!";
		$("#download_file__popup .modal-title-popup").html("Selected Documents");
		$("#download_file__popup .modal-button-popup").val("Download Zip");
	} else if(type == "email") {
		if($(".check_thisone234:checked").length > 0) {
			$("#send_documents_2").trigger("click");
		} else {
			message = "Please select documents to email!";
			alert(message);
		}
		return false;
	}
	if($(".check_thisone:checked").length > 0) {
		var ids = [];
		var html = "";
		html += "<table width='100%' class='table'>";
		html += "<tr><th>Document Name</th><th>Date</th></tr>";
		$('.check_thisone:checked').each(function () {
			var attr_id = $(this).attr("data-doc-id");
			var attr_date = $(this).attr("data-doc-date");
			...
			if(typeof href_data !== "undefined") {
				...
				var doc_ext = doc_name.split('.').pop();
				if(type == "merge" && (doc_ext == "pdf" || doc_ext == "docx")) {
					ids.push(doc_name);
					html += "<tr><th>"+doc_name+"</th><th>"+attr_date+"</th></tr>";
				} else if(type == "zip"){
					ids.push(doc_name);
					html += "<tr><th>"+doc_name+"</th><th>"+attr_date+"</th></tr>";
				}
		...
		html += "</table>";
		if(type == "merge" && ids.length < 1) {
			message = "Only Docx and Pdf documents are available for merge!";
			alert(message);
			return false;
		}
		$("#download_file__popup .modal-text").html(html);
		$("#download_file__popup").show();
		//$("#documents_operation_form").submit();
		$("#activity_operation_form #ids").val(ids.join(","));
		$("#activity_operation_form #operation").val(type);
	} else {
		alert(message);
		return false;
	}
...
// Get the modal
var modal = document.getElementById("download_file__popup");
