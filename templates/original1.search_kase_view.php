...
for ($int=0;$int<$numbs_case_type;$int++) {
	...
	$case_count = mysql_result($result_case_type, $int, "case_count");
...
$query = "SELECT * 
	FROM `ikase" . $data_source . "`.cse_casestatus cstat
	WHERE 1
	" . $order_by;
...
for ($int=0;$int<$numbs;$int++) {
	...
	$option = "<option value='" . $casestatus . "'>" . $casestatus . "</option>";
...
$query_sub = "SELECT * 
	FROM `ikase" . $data_source . "`.cse_casesubstatus csubstat
	WHERE 1
	" . $order_by;
...
for ($int_sub=0;$int_sub<$numbs_sub;$int_sub++) {
	...
	$option_sub = "<option value='" . $casesubstatus . "'>" . $abbr . " - " . $casesubstatus . "</option>";
	$casesubstatus_options .= "" . $option_sub;
}
