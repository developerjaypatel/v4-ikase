...
$result_case_type = mysql_query($query_case_type, $link) or die("unable to get venues<br>" . $query_case_type);
$numbs_case_type = mysql_numrows($result_case_type);
...
for ($int=0;$int<$numbs_case_type;$int++) {
	$case_type = mysql_result($result_case_type, $int, "case_type");
...
$query = "SELECT * FROM `cse_venue` 
WHERE 1
ORDER BY venue ASC";

$result = mysql_query($query, $link) or die("unable to get venues<br>" . $query);
$numbs = mysql_numrows($result);
$venue_options = "<option value=''>Select from List</option>";
for ($int=0;$int<$numbs;$int++) {
	$venue_id = mysql_result($result, $int, "venue_id");
	$venue_uuid = mysql_result($result, $int, "venue_uuid");
	$venue = mysql_result($result, $int, "venue_abbr");

	$option = "<option value='" . $venue_uuid . "'>" . $venue . "</option>";
	$venue_options .= "" . $option;
}
$query_cus = "SELECT data_source 
FROM ikase.cse_customer
WHERE customer_id = '" . $_SESSION["user_customer_id"] . "'";
$result_cus = mysql_query($query_cus, $link);

$data_source = mysql_result($result_cus, 0, "data_source");
/*
...
$result = mysql_query($query_status, $link) or die("unable to get status<br>" . $query_status);
$numbs = mysql_numrows($result);

if ($numbs==0) {
	//fill it from ikase
	$query = "INSERT INTO `" . $db_name . "`.cse_casestatus SELECT * FROM ikase.cse_casestatus";
	$result = mysql_query($query, $link) or die("unable to get status<br>" . $query);
	//requery
	$result = mysql_query($query_status, $link) or die("unable to get status<br>" . $query_status);
	$numbs = mysql_numrows($result);
}
...
for ($int=0;$int<$numbs;$int++) {
	$casestatus_id = mysql_result($result, $int, "casestatus_id");
	$casestatus_uuid = mysql_result($result, $int, "casestatus_uuid");
	$casestatus = mysql_result($result, $int, "casestatus");
	$law = mysql_result($result, $int, "law");
...
$result_sub = mysql_query($query_sub, $link) or die("unable to get sub stauses<br>" . $query_sub);
$numbs_sub = mysql_numrows($result_sub);

for ($int_sub=0;$int_sub<$numbs_sub;$int_sub++) {
	$casesubstatus_id = mysql_result($result_sub, $int_sub, "casesubstatus_id");
	
	$casesubstatus_uuid = mysql_result($result_sub, $int_sub, "casesubstatus_uuid");
	$casesubstatus = mysql_result($result_sub, $int_sub, "casesubstatus");
	$deleted = mysql_result($result_sub, $int_sub, "deleted");
	$law =mysql_result($result_sub, $int_sub, "law");
	if ($_SESSION["user_customer_id"]=="1070") {
		$abbr = mysql_result($result_sub, $int_sub, "abbr") . " - ";
  }
...
$result_sub_sub = mysql_query($query_sub, $link) or die("unable to get sub sub stauses<br>" . $query_sub);
$numbs_sub = mysql_numrows($result_sub_sub);

for ($int_sub=0;$int_sub<$numbs_sub;$int_sub++) {
	$casesubsubstatus_id = mysql_result($result_sub_sub, $int_sub, "casesubsubstatus_id");
	
	$casesubsubstatus_uuid = mysql_result($result_sub_sub, $int_sub, "casesubsubstatus_uuid");
	$casesubsubstatus = mysql_result($result_sub_sub, $int_sub, "casesubsubstatus");
	$deleted = mysql_result($result_sub_sub, $int_sub, "deleted");
	$law =mysql_result($result_sub_sub, $int_sub, "law");
	if ($_SESSION["user_customer_id"]=="1070") {
		$abbr = mysql_result($result_sub_sub, $int_sub, "abbr") . " - ";
  }
