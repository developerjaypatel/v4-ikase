...
<% if (homepage != true) { %>
<div style="float:right; width:40%; height:570px; padding-top:5px; padding-left:10px; display:none; background: url(img/glass_dark.png" id="preview_pane_holder">
    <div>
...
<div style="height:600px; overflow-y:scroll; width:100%" id="task_list_outer_div">
<% } else { %>
<div id="task_list_outer_div">
<% } %>
	<div class="glass_header" style="width:100%; height:45px">
        <div style="float:right" id="task_floats">
        <a id="hide_preview_pane"></a>
		<?php
		$day = date('w');
        ...
        <% if (title=="Task Inbox") { %>
            ...
            <% } %>
        <button class="btn btn-xs btn-warning" id="task_overdue" title="Click to list overdue tasks">Overdue Tasks</button>
        &nbsp;&nbsp;&nbsp;
        <button class="btn btn-xs btn-success" id="task_completed" title="Click to list completed tasks">Completed Tasks</button>
        <% } %>
        <!--
        <% if (title!="Task Inbox") { %>
        &nbsp;&nbsp;&nbsp;
        <button class="btn btn-xs btn-primary" id="open_tasks" title="Click to list open tasks">Open Tasks</button>
        <% } %>
        -->
        <% if (!homepage) { %>
        <span id="date_range_holder" class="white_text" style="display:none">
            Date Range <input id="start_dateInput" class="range_dates" value="<%=start %>" style="width:80px" /> through <input id="end_dateInput" class="range_dates" value="<%= end %>" style="width:80px" />
            &nbsp;
            <button class="btn btn-xs btn-primary" style="cursor:pointer; font-size:0.7em; visibility:hidden" id="update_date_range" title="Click to Update the Event Dates on this list">Update List</button>
        </span>
         &nbsp;&nbsp;&nbsp;
        <% } %>
        <% if (blnBadDates) { %>
        <span style="background:fuchsia; color:white; padding:2px; margin-right:250px" title="Please fix the date of the tasks indicated in pink">Please Fix Task Dates (Pink)</span>
        <% } %>
        <!--
        <a id="print_today_link" href="report.php#taskdayinbox/<%=today %>" target="_blank" title='Click to print today&rsquo;s tasks' style='cursor:pointer' class="white_text">Print Today</a>&nbsp;<% if (typeof arrDayCount[<?php echo date("mdy"); ?>] != "undefined") { %><span style='color:black; font-size:0.7em' title='There are <%=arrDayCount[<?php echo date("mdy"); ?>] %> tasks for this day'>(<%=arrDayCount[<?php echo date("mdy"); ?>] %>)</span><% } %><span id="print_weeks">&nbsp;&nbsp;&nbsp;<a id="print_thisweek_link" href="report.php#taskbydates/<%=week_start %>/<%=week_end %>" target="_blank" title='Click to print this week&rsquo;s tasks' style='cursor:pointer' class="white_text">Print This Week</a>&nbsp;&nbsp;&nbsp;<a id="print_nextweek_link" href="report.php#taskbydates/<%=next_week_start %>/<%=next_week_end %>" target="_blank" title='Click to print next week&rsquo;s tasks' style='cursor:pointer' class="white_text">Print Next Week</a></span>
        -->
        <div style="display:none">
        	<!--vestigial links, keeping so that a tags can "clicked"-->
            <a id="print_today_link" href="report.php#taskdayinbox/<%=today %>" target="_blank" title='Click to print today&rsquo;s tasks' style='cursor:pointer' class="white_text">Print Today</a>&nbsp;<% if (typeof arrDayCount[<?php echo date("mdy"); ?>] != "undefined") { %><span style='color:black; font-size:0.7em' title='There are <%=arrDayCount[<?php echo date("mdy"); ?>] %> tasks for this day'>(<%=arrDayCount[<?php echo date("mdy"); ?>] %>)</span><% } %>
            <span id="print_weeks">
                &nbsp;&nbsp;&nbsp;
                <a id="print_thisweek_link" href="report.php#taskbydates/<%=week_start %>/<%=week_end %>" target="_blank" title='Click to print this week&rsquo;s tasks' style='cursor:pointer' class="white_text">Print This Week</a>
                &nbsp;&nbsp;&nbsp;
                <a id="print_nextweek_link" href="report.php#taskbydates/<%=next_week_start %>/<%=next_week_end %>" target="_blank" title='Click to print next week&rsquo;s tasks' style='cursor:pointer' class="white_text">Print Next Week</a>
                &nbsp;&nbsp;&nbsp;
        ...
        <select id="task_print_options">
            ...
            <option value="today" id="today_option">Today</option>
             ...
             <option value="all" id="all_option">Date Range</option>
             <option value="" id="filtered_option" style="display:none"></option>
        </select>
        </div>
		<input type="hidden" name="case_id" id="case_id" value="<%= case_id %>" />
        <span style="font-size:1.2em; color:#FFFFFF">
        	<span id="task_form_title"><%=title%></span>
            <!--&nbsp;<div id="task_form_count" style="position: relative;left: 95px;padding-left:3px; margin-top:-25px; color:white; font-size:0.8em; width:20px">(<%=tasks.length %>)</div>-->
            <!-- solulab code start 19-04-2019-->
            &nbsp;<div id="task_form_count" style="position: relative;left: 115px;padding-left:3px; margin-top:-20px; color:white; font-size:0.8em; width:20px">(<%=tasks.length %>)</div>
            <!-- solulab code end 19-04-2019-->
        </span>
			<div style="position:relative; left:195px; margin-top:-22px; width:700px" id="task_manage_holder">
            	<!--
                ...
                <button id="open_tasks" type="button" class="btn btn-primary btn-sm" style="display:none; margin-top:-5px">Open Tasks</button>
                <button id="closed_tasks" type="button" class="btn btn-primary btn-sm" style="display:none; margin-top:-5px">Closed Tasks</button>
                <div id="deleted_tasks_holder" style="display:none">
                    <button id="deleted_tasks" type="button" class="btn btn-warning btn-sm" style="margin-top:-5px">Deleted Tasks</button>
                </div>
                <% } %>
            </div>
    ...
    <% if (count_report!="") { %>
    <div class="glass_header" style="width:100%; height:45px; margin-top:10px; text-align:right">
	    <span class="white_text"><div id="show_all_tasks" style="display:none; margin-right:10px;"><span id="show_all_link" style="cursor:pointer" title="Click to show all overdue tasks">Show All</span></div><%=count_report %></span>
    </div>
    <% } %>
    ...
    <table id="task_listing" class="tablesorter task_listing" border="1" cellpadding="0" cellspacing="0" style="width:100%">
       ...
       _.each( tasks, function(task) {
           ...
           var the_day = moment(task.task_dateandtime).format("MMDDYY");
        	...
        	%>
        <tr class="task_date task_date_<%=the_day %> task_row" id="row_id_<%=rowCounter %>">
        	<% rowCounter++; %>
            <td colspan="9">
        ...
        <% } %>
       	<tr class="task_data_row task_row_<%= task.id %> task_date_<%=the_day %> task_row" id="row_id_<%=rowCounter %>">
        	<% rowCounter++; %>
        ...
        <% if (!blnMyTasks) { 
            ...
            <table style="width:99.5%" border="0" align="left">
                ...
                <tr>
                    ...
                    width:<% if (homepage != true) { %>7%<% } else { %>3%<% } %>;
                    background:<% if (task.bad_date_indicator) { %>fuchsia<% } %>"
                    nowrap="nowrap">
                    ...
                    <% } %>
                    <% if (task.task_priority=="low" || task.task_priority=="") { %>
                    	<i class="glyphicon glyphicon-warning-sign" style="color:white;" title="Low Priority"></i>
                    <% } %>
                    <% if (task.deleted != "Y") { %>
                    	<a title="Click to edit task" class="list_edit open_task" id="open_task_<%= task.id %>_<%= task.case_id %>" style="cursor:pointer"><i style="font-size:15px;color:#06f; cursor:pointer;" class="glyphicon glyphicon-edit" title="Click to Open Task"></i></a>
                    <% } else { %>
	                    <span style="background:orange;color: black;padding: 2px;font-size: 1.2em;" title="Deleted by <%=task.track_user%> on <%= moment(task.track_date).format("MM/DD/YYYY") %>">DELETED</span>
                    <% } %>
                    	&nbsp;<a href="report.php#task/<%=task.id %>" target="_blank"><i style="font-size:15px;color:#CC99CC; cursor:pointer;" class="glyphicon glyphicon-print" title="Click to Print Task"></i></a><span style="margin-left:10px"  onmouseover="showAttachmentPreview('task', event, '<%=task.attachments%>', <%=task.case_id%>, <%=task.customer_id%>)" onmouseout="hideMessagePreview()">
                		<i style="font-size:15px;color:#FFFFFF; display:<%=attach_indicator%>" class="glyphicon glyphicon-paperclip"></i>
                	</span>
                     <% if (task.task_type=="closed") { %><span style="background:red; color:white; padding:2px" title="Closed by <%=task.track_user%> on <%= moment(task.track_date).format("MM/DD/YYYY") %>">CLOSED</span><% } %>
                    </div>
               ...
               <% } %>
                ...
                <td style="font-size:1.17em; width:<% if (homepage != true) { %>3%<% } else { %>3%<% } %>">
                    ...
                    <% if (arrCcUserNames.length > 0) { %>
                    <span class="cc_values" style="background:orange; color:white; padding:2px"><%= arrCcUserNames.join("; ") %></span>
                    <% } %>
                </td>
			   ...
			   <% if (title.indexOf("Kase") == -1) { %>
				   <td style="font-size:1.17em; width:<% if (homepage != true) { %>29.8%<% } else { %>23.5%<% } %>">
						...
						%>
                        <% if (task.case_number_link!="") { %>
                        Case #:<%= task.case_number_link %>&nbsp;&nbsp;Case <%= task.case_status %><br />
                        <% } %>
                        <% if (task.case_number_link=="" && task.file_number_link!="") { %>
                        File #:<%= task.file_number_link %>&nbsp;&nbsp;Case <%= task.case_status %><br />
                        <% } %>
                ...
                <td style="font-size:1.17em; width:<% if (homepage != true) { %>5%<% } else { %>5.5%<% } %>;">
                	<span id="type_holder_<%= task.id %>"><%= task.task_type.replace("_", " ").capitalizeAllWords() %></span>
                </td>
				<td style="width:5%" align="right">
                    ...
                    <?php } ?>
                    <div style="float:left">
                    <% if (task.task_type!="closed" && task.deleted != "Y") { %>
					<div style="float:right">
                    	<button class="btn btn-primary btn-xs close_task" role="button" id="close_<%= task.id %>" title="Click to mark Task as Completed.  The Task will still be available under Tasks|Completed Tasks">Close Task</button>
                    </div>
					<% } %>
                    <% if (task.deleted == "Y") { %>
                    <div style="float:right">
                    	<button class="btn btn-success btn-xs restore_task" role="button" id="restore_<%= task.id %>" title="Click to Restore Task">Restore Task</button>
                    </div>
                    <% } %>
                    </div>
					</td>
		        </tr>
                <% task.task_description = task.task_description.replace("background-color: rgb(255, 255, 51);", "background-color: rgb(255, 255, 51); color:black"); %>
        		<tr class="description_row" id="description_<%= task.id %>">
