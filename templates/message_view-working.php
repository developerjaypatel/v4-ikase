...
<style>
  /*adding this style for compose model starts from top in big screen - added by mukesh on 10-5-2023*/
  .modal-dialog{
    /*top:350px !important;*/
    /*top:50%;*/
  }
...
<!-- added above style, and changed table width 550 to 750 by mukesh on 2-6-2023 -->
<table width="750" border="0" align="left" cellpadding="2" cellspacing="0" id="message_holder_table">
  <tr>
  ...
  <tr>
    ...
    <td width="45%" id="message_to_td">
      ...
      <div class="user_listing" id="message_users_list_short"></div>
    </td>
  ...
  <tr>
    <th align="left" valign="top" scope="row" id="ktags_holder">Ktags:</th>
    <td colspan="2" valign="top" id="ktagstd">
      <select id="ktags_dropdown_list" name="ktags_dropdown_list" class="modal_input" style="margin-top:5px;">                
      </select>&nbsp;<button id="manage_ktags" class="btn btn-xs btn-primary manage_ktags">manage</button>
    </td>
  </tr>
...
var arrEmailAddressCc = [];
var arrEmailAddressBcc = [];
arrEmailAddressCc.push("<%= message_cc %>");
arrEmailAddressBcc.push("<%= message_bcc %>");

$("#token-input-message_ccInput").val(arrEmailAddressCc.join(","));
$("#token-input-message_bccInput").val(arrEmailAddressBcc.join(","));

$("#emailaddress_ccInput").val(arrEmailAddressCc.join(","));
$("#emailaddress_bccInput").val(arrEmailAddressBcc.join(","));

//according to screen height compose email model top position set - addded by mukesh on 5-6-2023
if($(window).innerHeight() > 1500)
{
  $(".modal-dialog").attr("style","top:550px !important");
}
else
{
  $(".modal-dialog").attr("style","top:350px !important");
}
messageUsersShort();
function messageUsersShort() {
    $('#message_users_list_short').fadeIn(function() {
      var mymodel = new Backbone.Model();
      mymodel.set("holder", "message_users_list_short");
      mymodel.set("source", "To");
      $('#message_users_list_short').html(new user_listing_message_short({collection: worker_searches, model: mymodel}).render().el);
      });
}

// function for loading ktags in drop down
function loadKtags()
{
  $.get("api/ktags/filters",{},function(data){
    data = JSON.parse(data);
    $("#ktags_dropdown_list").html("<option value='No Ktags'>Select Ktags</option>");
    for(i=0;i<data.length;i++){
      //alert(data[i].tag_id);
      var option = "<option value='"+ data[i].activity_ktags +"'>"+ data[i].activity_ktags +"</option>";
      $("#ktags_dropdown_list").append(option);
    }
  });
}
loadKtags();
