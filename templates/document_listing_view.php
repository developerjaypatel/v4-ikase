...
<div id="document_list_content" style="display:none">
    ...
    <% } %>
    	...
    	<div style="width:300px">
        	...
        	<div style="float:right; margin-top:4px">
            	<a href="#archives_merus/<%=kase.case_id %>" style="cursor:pointer" class="white_text">Merus Archives</a>&nbsp;<div id="archive_count" style="position: relative;left: 60px; padding-left:3px; margin-top:-25px; color:white; font-size:0.8em"></div>
            </div>
            <?php } ?>	
            <?php if ($blnArchives) { ?>
        	<div style="float:right; margin-top:4px">
    ...
    <div id="document_right_pane" style="float:right; display:none; margin-top:55px">
	...
	<div class="glass_header">
		<form id="documents_operation_form" method="POST" action="../merge_documents/documents.php">
		<input type="hidden" name="ids" id="ids"/>
		<input type="hidden" name="operation" id="operation"/>
		<input type="hidden" name="case_id" id="case_id" value="<%=kase.case_id %>"/>
		<button type="button" class="btn btn-success" id="my_button_0">
			<i class="glyphicon glyphicon-envelope" style="color:white"></i>
			<span style="color:white">Email Docs</span>
		</button>
		<button type="button" class="btn btn-success" id="my_button_1">
			<i class="glyphicon glyphicon-align-justify" style="color:white"></i>
			<span style="color:white">Merge Docs</span>
		</button>
		<button type="button" class="btn btn-success" id="my_button_2">
...
<!-- 
       ...
       _.each( kase_documents, function(kase_document) {
       	...
       	<tr class="kase_document_data_row kase_document_row_<%=kase_document.document_id%>" style="display:">
        	<td nowrap="nowrap">
            <input type="checkbox" style="margin-right:10px" data-doc-date="<%= kase_document.document_date %>" data-doc-id="<%=kase_document.document_id%>" id="check_assign_<%=kase_document.document_id %>" class="check_thisone check_thisone_<%=kase_document.document_id %>"/><a name="document_bill_<%=kase_document.document_id%>" id="document_bill_<%=kase_document.document_id%>" class="bill_icon" style="cursor:pointer" title="Bill this"><i class="glyphicon glyphicon-time" style="color:#F90">&nbsp;</i></a>
	        ...
	        <td><%= kase_document.document_date %><%= kase_document.user_name %></td>
...
<div id="download_file__popup" class="modal-popup">
  <div class="modal-content-popup">
	...
	<p class="modal-popup-button"><input type="button" class="modal-button-popup btn btn-success" onclick='$("#documents_operation_form").submit();$("#download_file__popup").hide();'/></p>
  </div>
...
$("#my_button_0").click(function() {
	show_modal_popup('email');
});
$("#my_button_1").click(function() {
	show_modal_popup('merge');
});
$("#my_button_2").click(function() {
...
function show_modal_popup(type) {
	...
	if($(".check_thisone:checked").length > 0) {
		...
		$('.check_thisone:checked').each(function () {
			...
			var doc_name = $("#document_name_"+attr_id).val();
		...
		$("#documents_operation_form #ids").val(ids.join(","));
		$("#documents_operation_form #operation").val(type);
	} else {
		alert(message);
		return false;
	}
