       	...
       	<tr class="kase_archive_data_row kase_archive_row_<%=kase_archive.archive_id%>">
        	<td nowrap="nowrap">
                ...
                var archive_number = kase_archive.doc_number.replace("A", "");
                 %>
                <a href="http://kustomweb.xyz/tritek/get_archive.php?db=<?php echo $_SESSION["user_data_source"]; ?>&recno=<%=kase_archive.recno %>&archive_number=<%=archive_number %>&case_id=<%=kase.id %>&cpointer=<%=kase.get("cpointer") %>&sess_id=<?php echo $_SESSION["user"]; ?>" class="white_text" target="_blank">
                	<%=kase_archive.document_name%>
                </a>
                <% } else { %>
                <!-- solulab code start - 17-04-2019-->
                    <a href="http://kustomweb.xyz/tritek/get_doc.php?db=<?php echo $_SESSION["user_data_source"]; ?>&recno=<%=kase_archive.recno %>&doc=<%=kase_archive.doc_number %>&case_id=<%=kase.id %>&cpointer=<%=kase.get("cpointer") %>&sess_id=<?php echo $_SESSION["user"]; ?>" class="white_text" target="_blank">
                        <%=kase_archive.document_name%>
                    </a>
