	...
	$.fn.datetimepicker = function( opt ) {
		var KEY0 = 48,
			...
			createDateTimePicker = function( input ) {
				if ($(".custom_dtp_height_indicator").length > 0) {
					var datetimepicker = $('<div '+(options.id?'id="'+options.id+'"':'')+' class="xdsoft_datetimepicker xdsoft_noselect custom_dtp_height '+options.className+'"></div>'),
				...
				calendar
					.on('click.xdsoft','td',function() {
						//var source_cell_number = $(".xdsoft td").html();
						//alert("working");
						//var formatted_date = moment(_xdsoft_datetime.end).format("MM/DD/YYYY");
						//alert(formatted_date);
						if ($("#event_dateandtimeInput").val() != undefined || $("#task_dateandtimeInput").val() != undefined) {
							var days_val = "0";
							var holiday_validation = "Y";
							//if (customer_id == "1033") {
								setTimeout( function() {
									var formatted_date = $("#event_dateandtimeInput").val();
									if (formatted_date == undefined) {
										formatted_date = $("#task_dateandtimeInput").val();
									}
									var arrDate = formatted_date.split(" ");
									//arrDate.splice(0, 1);
									//alert(arrDate[0]);
									...
									var formValues = "days=" + days_val + "&date=" + arrDate[0] + "&holiday_validation=" + holiday_validation;
									...
									$.ajax({
									  ...
									  success:function (data) {
										  if(data.error) {  // If there is an error, show the error tasks
											  ...
											  if (data[0].calculated_date != data[0].start_date) {
												var text_choice;
												var question = confirm("This is a holiday, are you sure?");
												if (question == true) {
													datetimepicker.trigger('close.xdsoft');
												} else {
													$("#event_dateandtimeInput").val(data[0].calculated_date + " " + arrDate[1]);
													datetimepicker.trigger('close.xdsoft');
												}
						...
						if( options.onSelectDate &&	options.onSelectDate.call ) {
							...
							//alert(options.onSelectDate.call(datetimepicker,_xdsoft_datetime.currentTime,datetimepicker.data('input')));
						}
						
						datetimepicker.data('changed',true);
						//alert(datetimepicker.data());
						datetimepicker.trigger('xchange.xdsoft');
