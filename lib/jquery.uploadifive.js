    ...
    var a = {
        init: function(c) {
            return this.each(function() {
				...
				var buttonText = "<i class='glyphicon glyphicon-plus'></i>Add File...";
				var width = 100;
				var hash = document.location.hash;
				if (document.location.hash.indexOf("#kase/")==0 || document.location.hash.indexOf("#parties/")==0) {
					//buttonText = "Choose/Drop Upload(s)";
					//width = 200;
				}
                ...
                ;
                if($(".modal").hasClass("in") && $(".message_attach_form #queue").length > 0) {
                    var hash = document.location.hash;
                    
                    //if (hash == "" ) {
                        setTimeout(function(){ 
                            if( $("#case__input_ui ul.token-input-list-facebook").css('display') == 'none') {
                                $("#case__input_ui ul.token-input-list-facebook").attr("style", "display:none;width : 337px !important; ");
                            } else {
                                $("#case__input_ui ul.token-input-list-facebook").attr("style", "width : 337px !important; ");
                            }
                    ...
                    setInterval(function(){ 
                        if(document.location.hash.indexOf("#tasks/")==0 && $("#uploadifive-file_upload").length > 0) {
                                $("#uploadifive-file_upload").attr("style", "height: 30px;line-height: 30px;overflow: hidden;position: relative;text-align: center;width: 100px;");
                        }
                    }, 50);
                }
                var filename = false;;
                ...
                d.addQueueItem = function(k) {
					if ($(".message_attach_form #queue").length > 0) {
						if ($(".message_attach_form #queue").css("display")=="none") {
							$(".message_attach_form #queue").show();
                        }
                        if($(".modal").hasClass("in") && $("#send_queue").length > 0) {
                    ...
                    if (b.inArray("onAddQueueItem", f.overrideEvents) < 0) {
                        ...
                        /*
		if (typeof d.queueItem == "undefined"){
			//already deleted
			return false;
		}
		*/
                        ...
                        if($(".modal").hasClass("in") && $("#send_queue").length > 0) {
                            var filename = m;
                            var current_filename = m;
                            var filenamehref = $('<div>').append(filename).find('a:first').attr('href');
                            ...
                            if (current_case_id != "") {
                                current_filename = current_case_id + "/" + current_filename;
                            }
                            var path = "D:/uploads/" + customer_id + "/" + current_filename;

                            var str_sep = '';
                            if($("#send_queue div.comma_sepr_file_div").length > 0) {
                                str_sep = ', ';
                            }
                            //console.log("LIBRARY 244 --> Length -->"+$("#send_queue div.comma_sepr_file_div").length);
                            $("#send_queue").append("<div class='comma_sepr_file_div' style='float: left;margin-right: 5px;'>"+str_sep+"<a title='"+m +"' href='" + path + "' target='_blank' class='white_text'>" + filename + "</a></div>");
                        }
                    }
                    ...
                    if ($(".message_attach_form").length > 0) {
                        //we're attaching to a note or event or task
                    ...
                    if ($('#image_holder').length > 0) {
                        $('.person_image_form #file_upload').uploadifive('upload');
                    }
                    if ($('#image_holder').length > 0) {
                        $('.personal_injury_image_form #file_upload').uploadifive('upload');
                    }
                    if ($('.import #file_upload').length > 0) {
                        $('.import #file_upload').uploadifive('upload');
                    }
                }
                ;
                d.removeQueueItem = function(m, l, k) {
                    ...
                    var j = l ? 0 : 500;
                    if (m.queueItem) {
                        if (m.queueItem.find(".fileinfo").html() != " - Uploaded") {
                ...
                d.checkExists = function(k) {
                    if (b.inArray("onCheck", f.overrideEvents) < 0) {
                        ...
                        b.post(f.checkScript, j, function(l) {
                            var arrReturn = l.split("|");
                            k.exists = parseInt(arrReturn[0]);
                            k.exists_message = arrReturn[1];
                        });
                        if (k.exists) {
                            if (!confirm("A file named " + k.name + " already exists in your upload folder.\n" + k.exists_message + "\n\n\Please click 'OK' to overwrite it, or 'Cancel' to stop the upload")) {
                                a.cancel.call(g, k);
                                return true;
                            }
                        }
                ...
                d.uploadComplete = function(l, j, k) {
                    if (b.inArray("onUploadComplete", f.overrideEvents) < 0) {
                        ...
                        //var current_filename = j.queueItem.find(".filename").html();
                        var current_filename = j.xhr.responseText;
                        //not for eams forms
                        if (j.queueItem.parent().parent().hasClass("eams_form_attach_form")) {
                            j.queueItem.find(".filename").html("<a href='eams_forms/" + current_filename + "' target='_blank' title='Click to review imported document'>" + current_filename + "</a>");
                        } else {
                            if (current_case_id == -1) {
                                current_case_id = "";
                            }
                            ...
                            j.queueItem.find(".filename").html("<a href=https://v4.ikase.org/document_read.php?file=D:/uploads/" + customer_id + "/" + current_filename + " target='_blank' title='Click to review imported document'>" + current_filename + "</a>");
                        }
                        j.queueItem.find(".fileinfo").html(" - Uploaded");
                        //at this point, switch back the save icon
                        $("#gifsave").fadeOut(function() {
                            $("#modal_save_holder").fadeIn();
                        });
                        //if we are in settings mode, hide the upload, and transfer the filename to the value field
                        if ($('.setting #file_upload').length > 0) {
                            $("#setting_attachments").fadeOut();
                            $("#setting_default_value").fadeOut();
                            $("#setting_valueInput").val(current_filename);
                        }
                ...
                if (c) {
                    ...
                    if ((e.uploads.count + e.uploads.current) < f.uploadLimit || f.uploadLimit == 0) {
                        ...
                        b("#" + f.queueID).find(".uploadifive-queue-item").not(".error, .complete").each(function() {
                            ...
                            if (f.checkScript) {
                                _file.checking = true;
                                var div_id = _file.queueItem[0].id;
