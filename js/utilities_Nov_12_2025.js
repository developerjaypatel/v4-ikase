...
function displaySavedInfo(formValues, form_name) {
	...
	for(var i =0, len = arrValues.length; i < len; i++) {
		...
		if ($("." + form_name + " #" + field_name + "Input").length != 0) {
			...
			var isFirstTargetInput = ""
			if(form_name == 'settlement')	{ // in settlement it should update only first element, settlement has duplicate ids 
				 isFirstTargetInput = ":first"
			}
			...
			if (!blnSkipField) {
				...
				if ($("." + form_name + " #" + field_name + "Input").prop("type")!= "select-one") {
					if (field_name!="note" && field_name!="signature") {
						//check for checkbox
						if ($("." + form_name + " #" + field_name + "Input").prop("type")!= "checkbox") {
							$("." + form_name + " #" + field_name + "Span" + isFirstTargetInput).html(escapeHtml($("." + form_name + " #" + field_name + "Input").val()));
						} else {
							//it is a checkbox
							span_value = $("." + form_name + " #" + field_name + "Input" + isFirstTargetInput).is(':checked');
							if (span_value) {
								span_value = "<span style='font-family:Wingdings; color:white'>ü</span>";
							} else {
								span_value = '';
							}
							$("." + form_name + " #" + field_name + "Span" + isFirstTargetInput).html(span_value);
						}
					} else {
						//the note is not escaped, except for script
						var note_value = $("." + form_name + " #" + field_name + "Input" + isFirstTargetInput).val();
						...
						$("." + form_name + " #" + field_name + "Span" + isFirstTargetInput).html(note_value);
					}
				} else {
					//text value of the drop down
					var dropdown_text = $("." + form_name + " #" + field_name + "Input :selected").text();
					...
					$("." + form_name + " #" + field_name + "Span" + isFirstTargetInput).html(escapeHtml(dropdown_text));
				}
			}
		}
		//toggleFormEdit(form_name);
