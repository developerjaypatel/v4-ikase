window.message_attach = Backbone.View.extend({
    ...
    render:function () {
		...
		setTimeout(function() {
			...
			if (document.location.hash.indexOf("#kases/")==0 || document.location.hash.indexOf("#kase/")==0 || document.location.hash.indexOf("#parties/")==0 || document.location.hash.indexOf("#partielist/")==0 || document.location.hash.indexOf("#exams/")==0) {
				...
				$(".message_attach_form #queue").css("width", "175px");
			...
			if (document.location.hash.indexOf("#tasks/")==0) {
				...
				$("#uploadifive-file_upload").css("height", "50px");
	...
	doTimeouts: function(event) {
		...
		if (self.model.get("document_id") != "" && typeof self.model.get("document_id") != "undefined") {
			...
			$("#message_attachments").removeAttr("style");
			...
			_.each( arrID, function(document_id) {
				...
				send_document.fetch({
					success: function(data) {
						...
						var filename = kase_document.document_filename;
						filename = filename.substring(0, 5) + " ...";
						//console.log("ATTACH 120 --> Length -->"+$("#send_queue div.comma_sepr_file_div").length);
						var str_sep = '';
	...
	showCaseDocuments: function(event) {
		...
		/*$('#myModal4 .modal-dialog').animate({width:the_width, marginLeft: the_left + "px"}, 100, 'easeInSine', 
		function() {
		...
		$('#myModal4 .modal-dialog').css("width", the_width);
		$('#myModal4 .modal-dialog').css("margin-left", the_left + "px");

		$('#message_documents_list').show();
		$("#message_documents_list").attr("style", "width: 30%; float: right; border: 1px solid white; overflow-y: auto; max-height: 650px;");
		//$("#message_holder_table").attr("width", "65%");
		$('#message_documents_list').show();
	},
