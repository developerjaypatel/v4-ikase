...
window.message_view = Backbone.View.extend({
	...
	initialize:function () {
	 ...
	 events:{
		...
		"click #manage_ktags": 						"manageKtags",
    },
	...
	doTimeouts: function (event) {
		var self = this;
		self.showSignature(); // added by mukesh on 10-5-2023 for showing signature in New Message Copose
	...
	messageUsers: function(event) {
		...
		if ($('#message_users_list').css("display")=="none") {
			the_left -= 200;
			the_width += 455;
			// left (80) and with (420) updated by mukesh on 7-6-2023 as issue of user list and case document list tother did not align properly
		}
		...
		function() {
			...
			$('#message_users_list').fadeIn(function() {
				...
				// setTimeout(function() {
				// 	var total_user = $('.message_user').length;	
				// 	if(total_user > 1)
				// 	{
				// 		var cnt = 0;
				// 		$('.message_user').each(function(){
				// 			cnt++;
				// 			if(cnt < 3)
				// 			{
				// 				$(this).trigger("click");
	...
	noSignature: function() {
		...
		// added by mukesh on 11-5-2023 for click on no signature, modal should be hide
		$("#myModal4").hide();
		$(".modal-backdrop").hide();
	},
	showSignature: function() {
		...
		signature.fetch({
			success: function (signature) {	
				// added href link of a tag in below code by mukesh on 11-5-2023 for redirecting to email setting pag for adding signature
				var no_signature = '<div id="signature_offer_holder" style="float:right; padding-right:45px"><a href="v8.php#emailsettings" id="signature_offer" class="white_text" style="cursor:pointer; color:red; background:white" title="Click if you do not want to add your signature to this email">No Signature</a></div>';
				...
				// added by mukesh on 11-5-2023 when signature added, 'No Signature' option should not show
	...
	manageKtags: function (event) {
	    // alert(event);
	    event.preventDefault();
	    var element = event.currentTarget;
	    var element_id = element.id;

	    $('.modal-dialog').animate({ width: "1180px", marginLeft: "-580px" }, 1100, 'easeInSine',
	      function () {        
	        var ktagsfilters = new KtagsFilters();	        
	        ktagsfilters.fetch({
	          success: function (data) {	          	
	            var mymodel = new Backbone.Model();
	            // $("#side_holder").show();
	            ...
	            $('#message_users_list').html(new ktags_listing({ collection: data, model: mymodel }).render().el).show();	          	
	          }
	        });
	      }
	    );
...
window.ktags_listing = Backbone.View.extend({
	initialize: function () {
	},
	events: {
		/*"click #select_all_filters":		"selectAll",*/
		"click #new_ktags_button": "newKtags",
		"click #save_ktags": "saveKtags",
		"click .ktags_checkbox": "activateKtags",
		"click .ktags_cell": "editKtags",
		"click .ktags_save": "updateKtags",
		"click #show_all_ktags": "showAllKtags"
	},
	render: function () {
		if (typeof this.template != "function") {
			this.model.set("holder", "manage_ktags_holder");
			var view = "ktags_listing";
			var extension = "php";
		...
		_.each(arrFilters, function (ktags) {
			if (typeof ktags.tag_id != "undefined") {
				var index = ktags.tag_id;
				var checked = " checked";
				var row_display = "";
				var cell_display = "color:white";
				var row_class = "active_filter";

				//var activity_ktags = ktags.activity_ktags.replace("_"," ");
				var activity_ktags = ktags.activity_ktags;			
					
				var input = "<input class='hidden' type='text' id='ktags_value_" + index + "' value='" + activity_ktags + "' />&nbsp;<button class='btn btn-xs btn-success ktags_save hidden' id='ktags_save_" + index + "'>Save</button>";
				
				var therow = "<tr class='" + row_class + "' style='" + row_display + "'><td class='kase_status'><input type='checkbox' class='ktags_checkbox hidden' value='Y' title='Uncheck to stop using this Ktags.' id='ktags_" + index + "' name='ktags_" + index + "'" + checked + "></td><td class='ktags' style='" + cell_display + "'><span id='ktags_span_" + index + "' class='ktags_cell' style='cursor:pointer' title='Click to edit this Ktags'>" + activity_ktags + "</span>" + input + "</td></tr>";
				arrRows.push(therow);									
			}
		});
		var html = "<table id='ktags_table'>" + arrRows.join("") + "</table>";
		try {
	...
	editKtags: function (event) {
		...
		var ktags_id = arrID[arrID.length - 1];

		//not open nor closed
		var current = $("#ktags_value_" + ktags_id).val();
		if (current == "Open" || current == "Closed") {
			return;
		}
		$("#ktags_span_" + ktags_id).fadeOut();
		$("#ktags_value_" + ktags_id).toggleClass("hidden");
		$("#caseTypeInput" + ktags_id).toggleClass("hidden");
		$("#ktags_save_" + ktags_id).toggleClass("hidden");
		$("#ktags_" + ktags_id).toggleClass("hidden");
	...
	newKtags: function (event) {
		event.preventDefault();
		$("#new_ktags_button").fadeOut();
		$("#new_ktags_holder").fadeIn(function () {
			$("#new_ktags").focus();
		});
	},
	saveKtags: function (event) {
		event.preventDefault();
		var url = 'api/ktags/add';
		var mymodel = this.model.toJSON();
		var ktags_id = mymodel.tag_id;
		
		var ktags = $("#new_ktags").val();
		var formValues = "ktags=" + encodeURIComponent(ktags);

		if (ktags == "" || ktags == null) {
			alert('Please enter Ktags');
		} else {
			$("#save_ktags").hide();
			$.ajax({
				url: url,
	...
	activateKtags: function (event) {
		var element = event.currentTarget;

		var arrID = element.id.split("_");
		var ktags_id = arrID[arrID.length - 1];
		document.cookie = "changes_need_render_new_kase_view=1";
		$("#ktags_save_" + ktags_id).trigger("click");
	},
	updateKtags: function (event) {
		event.preventDefault();
		var element = event.currentTarget;

		var arrID = element.id.split("_");
		var ktags_id = arrID[arrID.length - 1];
		var checkbox = document.getElementById("ktags_" + ktags_id);
		var deleted = "Y";
		if (checkbox.checked) {
			deleted = "N";
		}
		var ktags = $("#ktags_value_" + ktags_id).val();
		
		var mymodel = this.model.toJSON();		

		var url = 'api/ktags/update';
		var formValues = "ktags_id=" + ktags_id + "&ktags=" + encodeURIComponent(ktags) + "&deleted=" + deleted;
