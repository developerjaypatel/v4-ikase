...
window.kase_nav_bar_view_mobile = Backbone.View.extend({
	...
	searchKases: function (event) {
		...
		if (key.length > 0) {
			if (key.length > 2) {
				var my_kases = new KaseCollection(this.model.searchCollection(key));
				...
				if ($("#list_kases_header").length==0 || $("#list_kases_header").parent().parent()[0].id=="search_results") {
					...
					//$("#search_results").css("margin-top", "10px");
