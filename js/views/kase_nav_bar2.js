...
window.kase_nav_bar_view = Backbone.View.extend({
	...
	events: {
		...
		"click #eamsfiler_submissions": "listJetfiles2",
	...
	render: function () {
		...
		//Google Drive Plugin - START 2021-11-02 : 09:45
		setTimeout(function() {
			var CLIENT_ID = '959578964327-i7nilkm6ge3l5k0frpjhhtlg65reglrp.apps.googleusercontent.com';
			var API_KEY = 'AIzaSyAMWH4VgtxlQqv3MfK_uykZhHI6nyUWQFk';
			...
			function handleClientLoad() {
				...
				gapi.load('picker', initClient);
			}


			$(document).on('click', '#google_signin_menu', function(){
				handleAuthClick();
			});
			  ...
			  function updateSigninStatus(isSignedIn) {
				// alert(isSignedIn);
				if (isSignedIn) {
				  $('#google_signin_menu').html('<a id="google_authorize_button" title="Click to Sign-out from Google" style="cursor:pointer">Google Sign-out</a>');
				  oauthToken = gapi.auth2.getAuthInstance().currentUser.get().getAuthResponse().access_token;
				  
				} else {
				  $('#google_signin_menu').html('<a id="google_authorize_button" title="Click to SignIn via Google" style="cursor:pointer">Google Sign-In</a>');
				  oauthToken = 'Authorize';
				}
				
				setCookie('g_access_token', oauthToken, '1');
			  }

				function handleAuthClick() {
					if(oauthToken == 'Authorize'){
