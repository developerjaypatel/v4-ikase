window.kaseApplicantView = Backbone.View.extend({
	...
	/*
	events:{
        "click .applicant .delete":				"deleteApplicant",
		"click .applicant .save":				"addApplicant",
		"click .applicant .save_field":			"saveApplicantField",
		"click .applicant .edit": 				"toggleApplicantEdit",
		"click .applicant .reset": 				"resetApplicantForm",
		"click .applicant .calendar": 			"showCalendar",
		"keyup .applicant .input_class": 		"valueChanged",
		"dblclick .applicant .gridster_border": "editKaseApplicantField"
    },
	*/
    ...
    render: function () {
        ...
        $('#edit_applicant_holder', this.el).html(new applicantView({model:this.model}).render().el);
	...
	/*
	editKaseApplicantField: function (event) {
	...
	saveApplicantField: function (event) {
		...
		var theclass = event.currentTarget.parentElement.parentElement.parentElement.parentElement.parentElement.className;
	...
	addApplicant:function (event) {
		...
		$blnValid = $("#applicant_form").parsley('validate');
		
		//get out if invalid			
		if (!$blnValid) {
			$(".applicant .alert-warning").show();
			$(".applicant .alert-text").html("Please fill in the required fields in the correct format.");
			return;
		}
		
		$(".alert-warning").hide();
		
		var id = $('.applicant #id').val();
		if (id > 0) {
...
window.applicantView = Backbone.View.extend({
	...
	valueApplicantViewChanged: function(e) {
		//console.log(arguments[0].currentTarget.id);
		var source = arguments[0].currentTarget.id;
		source = source.replace("Input", "");
