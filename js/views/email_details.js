...
window.email_view = Backbone.View.extend({
	render: function () {
		...
		setTimeout(function(){ 
			...
			for(var i=0; i<= countArray; i++){
				...
				$('.emailTable').append('<tr id="er'+ json[i].email_id + '"><td>'+json[i].email_address+'</td><td class="email_status_'+json[i].email_id+'">'+bb+'</td><td><a class="deleteEmail" style="cursor:pointer" onclick=deleteEmail("'+json[i].email_id+'")><i style="color:#FF3737; cursor:pointer" class="glyphicon glyphicon-trash" title="Click to delete email"></i></a></td></tr>');
			}
		}, 3000);
	...
	scheduleAddEmail:function(event) {
		...
		//added on 13-4-2023 by mukesh for refreshing page after email added
