...
window.Router = Backbone.Router.extend({
    routes: {
		...
		//"kases/events/edit/phone/:case_id/:event_id/:case_number/:event_kind": "kaseEventTypeDialog",
    ...
    initialize: function () {
		...
		worker_searches = new WorkerSearchCollection();
		worker_searches.fetch({
			success:function(data){
				//console.log("carriers:" + data.length);
			}
		});
	...
	kasePhone: function (id, event_id) { 
		/*
		var event_id = event_id;
		var case_id = id;
		var case_number = case_number;
		var event_kind = "phone_call";
		//document.location.href = "#kases/events/edit/phone/" + case_id + "/" + event_id + "/" + encodeURIComponent(case_number) + "/" + event_kind;
		*/
		var self = this;
		var kase = kases.get(id);
		kase.set("header_only", true);
		$('#content').html(new kase_view({model: kase}).render().el);
		setTimeout(function() {
			self.kaseEventTypeDialog(id, event_id, "phone_call");
		}, 100);
	},
	...
	kaseEventTypeDialog: function (id, event_id, event_kind) {
		//alert("angel");
		readCookie();
		var self = this;
		var blnProceed = true;
		...
		occurences.fetch({
				success: function(data) {
					...
					if (event_kind == "phone_call" && event_id==-1) {
						occurence.set("title", kase.get("case_number") + " - " + moment().format("MM/DD/YYYY"));
					}
					occurence.set("case_number", kase.get("case_number"));
					occurence.set("case_uuid", kase.get("uuid"));
					occurence.set("gridster_me",true);
	...
	kaseNotes: function (case_id) {		
		...
		notes.fetch({
			success: function(data) {
				if (data.length=="0") {
					document.location.href = "#notes/" + case_id + "/-1/new";
					return;
				}
		...
		success:function (data) {
			if(data.error) {  // If there is an error, show the error messages
						...
						if (field_name!="password") {
							...
							if ($("." + form_name + " #" + field_name + "Input").prop("type")!= "select-one") {
								...
								if ($("." + form_name + " #" + field_name + "Input").prop("type")!= "checkbox") {						
									...
									if (span_value) {
										span_value = '<i class="glyphicon glyphicon-thumbs-up"></i>';
									} else {
										span_value = '<i class="glyphicon glyphicon-thumbs-down"></i>';
									}
				...
				if (form_name == "kase") {
					...
					customer_settings.get("case_number_next") = data.case_number_next;
