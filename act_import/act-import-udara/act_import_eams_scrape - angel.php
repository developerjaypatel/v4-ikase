...
function scrapeEams($adj_number) {
	...
	try {
		...
		//die($firm_name);
		$email = "latommy1@gmail.com"; //$customer->cus_email;
		if ($email=="") {
			$email = "nick@kustomweb.com";
		}
		$url = 'https://eams.dwc.ca.gov/WebEnhancement/';
		$fields = array("UAN"=>"", "firstname"=>$first_name, "lastname"=>$last_name, "email"=>$email, "reason"=>"CASESEARCH");
		//die(print_r($fields));
		$fields_string = "";
		...
		if ($headers["http_code"]==302) {
		   //redirect
			...
			$url = "https://eams.dwc.ca.gov/WebEnhancement/InjuredWorkerSearchScreen.jsp";
...
$sql_cases .= " rolodex ON (app.parent_person_uuid = rolodex.person_uuid AND rolodex.person_uuid = rolodex.parent_person_uuid)
			...
			if (count($arrValues) > $party_idx - 1) {
				...
				for($idx = $party_idx; $idx < count($arrValues); $idx = $idx + 3) {
					if (isset($arrValues[$idx]))
					{
					 $partie_name = $arrValues[$idx];
					}
					if (isset($arrValues[$idx + 1]))
					{
					 $partie_role = $arrValues[$idx + 1];
					}
					if (isset($arrValues[$idx + 2]))
					{
