...
if (isset($_GET["image"])) {
	$conf['default_image']				= $_GET["image"];
} else {
	$conf['default_image']				= 'separator.png';
	$conf['default_image']				= $_SERVER["DOCUMENT_ROOT"] . "/uploads/separator_pages80_2.png";
}
...
$outfile = $_SERVER["DOCUMENT_ROOT"] . "/uploads/th_100_test.png";
//die($filesize . "<br />");
if ($filesize < 2000) {
	//let's crop
	//exec("convert -define jpeg:size=200x200 " . $filename . "  -thumbnail 100x100^ -gravity center -extent 100x100 " . $outfile);
	
	exec("convert -resize '500^>' " . $filename . " -gravity center -crop 500x100+0+0 " . $outfile);
...
//global $max_color;
	...
	//die(print_r(Array($min_disp,$min_disp_at)));
