...
try {
	...
	HAVING COUNT(ccase.case_number) > 0
...
foreach($statutes as $index=>$statute){
	...
	if($index == 0) {
		$previous_date = 0;
   		$first_date = date("m/01/Y", strtotime($statute->sol));
	...
	if ($current_date != $previous_date && $previous_date !=0) {
		...
		<tr>
			...
			"<div style='float:right; cursor:pointer; color:white'><a href='statutes_month.php?month=" . date("m/d/Y", strtotime($statute->sol)) . "' style='color:white' target='_blank'>Print</a></div></td>
...
//	if ($current_date > $previous_date && $previous_date !=0 && date("m", strtotime($statute->sol)) == 06) {
//		$row = "";
//	}
	$arrRows[] = $row;
