...
function getClientEmails($sortby = "", $customer_id) {
	$sql = "SELECT DISTINCT pers.person_id id, 'applicant' `type`, pers.full_name, pers.first_name, pers.last_name, REPLACE(pers.email, ' ', '') email
	FROM cse_case ccase
	...
	SELECT DISTINCT corp.corporation_id id, 'plaintiff' `type`, corp.full_name, corp.first_name, corp.last_name, REPLACE(corp.email, ' ', '') email
	FROM cse_case ccase
...
$cases = getClientEmails($sortby, $customer_id);
...
$arrOutputColumns = array("id", "type", "full_name", "first_name", "last_name", "email");
...
//die(print_r($list));
//output file
$filename = 'D:/uploads/' . $customer_id . '/exports/clients_' . date('mdy') . '.csv';
$fp = fopen($filename, 'w');
