...
$sendit = passed_var("submit");
$sendRTR = passed_var("sendRTR");
...
<?php } ?>
<?php if ($sendRTR=="Send to RTR") {  ?>
<div style="background:white; text-align:center;display:none" id="matrix_holder"><img src="https://www.ikase.website/img/RTR_report_logo.jpg" height="200" alt="Realtime"></div>
<?php } ?>
<table width="900" border="0" align="center" cellpadding="3" cellspacing="0" style="margin-top:0px">
          ...
          <table>
            <tr>
              <td align="left" style="border-bottom:1px solid #CCC">
                <div style="font-size:1.2em; font-weight:bold; padding-bottom:5px">Send this Kase to Matrix &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" name="submit" value="Send It" /></div>
			...
			<tr>
              <td align="left" style="padding-top:20px">
                <input type="submit" style="background-color:#90ee90;" name="sendRTR" value="Send to RTR" />
                </td>
              </tr>
            <tr>
              <td align="left">
  ...
  <tr>
    ...
    <td width="10%" rowspan="4" valign="top">
        ...
        <?php if ($document_filename!="") { ?>
        <img src='https://www.ikase.website/uploads/<?php echo $customer_id ?>/<?php echo $case_id; ?>/<?php echo $document_filename ?>' class='applicant_img' style="border:1px solid white" width='200' height='200'>
        <?php } ?>
        </div></td>
  </tr>
   <tr>
...
function previewDoc(case_id, document_filename, id, type, thumbnail_folder) {
	var href = "https://www.ikase.website/api/preview.php?demo=&cusid=<?php echo $_SESSION["user_customer_id"]; ?>&case_id=" + case_id + "&file=" + encodeURIComponent(document_filename) + "&id=" + id + "&type=" + encodeURIComponent(type) + "&thumbnail_folder=" + encodeURIComponent(thumbnail_folder);
	//window.open(href);
...
$fp = fopen($upload_dir_config . '/demographics.html', 'w') or die('Cannot open file');
fwrite($fp, $content);
fclose($fp);
...
if ($emailit=="y") {
	...
	$file_url = "https://www.ikase.website/uploads/" . $customer_id . "/" . $case_id . "/demographics.html";
	...
	if ($sendRTR="Send to RTR"){
	$activity = "Demographics Sheet for Case " . $case_id . " Sent to Realtime By " . $_SESSION["user_name"];
	}
	if ($targetFile!="") {
	...
	if ($customer->inhouse_id > 0) {
		...
		if ($sendRTR="Send to RTR"){
		$filename = "https://www.realtime.com/dis/pws/manage/request/forward_ikase.php?cus_id=" . $customer_id . "&customer_name=" . urlencode($_SESSION['user_customer_name']) . "&case_id=" . $case_id . "&case=" . urlencode($case_name) . "&attachment=" . $targetFile . "&specific_instructions=" . urlencode($specific_instructions) . "&dmsauth=" . $dmsauth . "&kase_attachments=" . $kase_attachments;
		}
		//echo $filename;
...
$stmt = null; $db = null;
	...
	if ($targetFile!="") {
		$html_message .= "Attachment:<br>https://www.ikase.website/uploads/" . $customer_id . "/" . $case_id . "/" . urlencode($targetFile) . "<br>";
	}
	...
	if ($sendRTR="Send to RTR"){
	$to_name = "rtrorders@gmail.com";
	//$to_name = "nick@kustomweb.com";
	//$url = "https://www.realtime.com/dis/sendit.php";
	$url = "https://www.matrixdocuments.com/dis/sendit.php";
	}
