...
for ($x=0;$x<$numbs;$x++) {
	...
	$kai_info = mysql_result($result, $x, "kai_info");
	$sub_in = mysql_result($result, $x, "sub_in");
	...
	$defendant = mysql_result($result, $x, "defendant");
	$defendant_address = mysql_result($result, $x, "defendant_address");
	$defendant_phone = mysql_result($result, $x, "defendant_phone");
	$defendant_fax = mysql_result($result, $x, "defendant_fax");
	$defendant_site = mysql_result($result, $x, "defendant_site");
	$defendant_email = mysql_result($result, $x, "defendant_email");
	if ($defendant_phone!=""){
	...
	$special_instructions = mysql_result($result, $x, "special_instructions");
	...
	$file_number = mysql_result($result, $x, "file_number");
	
	//for now
	if ($case_number=="" && $file_number!="") {
		$case_number=$file_number;
	}
	$case_name = mysql_result($result, $x, "case_name");
	if ($case_name=="") {
...
for ($int=0;$int<$numbs_noemp;$int++) {
	...
	$partie_type_option = mysql_result($result_noemp, $int, "party_type_option");
	...
	if ($partie_type == "Plaintiff") { 
		//die("here");
		$plaintiff_kai_info = mysql_result($result_noemp, $int, "kai_info");
...
$result_pinj = mysql_query($sql, $link) or die("unable to get pinj<br>" . $sql);
$numbs_pinj = mysql_numrows($result_pinj);

for ($int_inj=0;$int_inj<$numbs_pinj;$int_inj++) {
	$injury_id = mysql_result($result_pinj, $int_inj, "personal_injury_id");
	$injury_uuid = mysql_result($result_pinj, $int_inj, "personal_injury_uuid");
	$personal_injury_date = mysql_result($result_pinj, $int_inj, "personal_injury_date");
	$personal_injury_info = mysql_result($result_pinj, $int_inj, "personal_injury_info");
	$sol_date = mysql_result($result_pinj, $int_inj, "statute_limitation");
