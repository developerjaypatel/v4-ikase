...
$header_start_time = 0;
$arrTiming = array();
$blnDebug = false; //($_SERVER['REMOTE_ADDR']=='173.55.229.70'); 
//($_SESSION['user_nickname']=="MA");	//($_SERVER['REMOTE_ADDR']=='173.55.229.70');	//($_SESSION['user_customer_id']=="1057");	//($_SERVER['REMOTE_ADDR']=='173.55.229.70');
if ($blnDebug) {
	$time = microtime();
	$time = explode(' ', $time);
	$time = $time[1] + $time[0];
...
$sql = "SELECT DISTINCT 
			...
			venue.venue_uuid, IF(venue.venue IS NULL, '', venue.venue) venue, venue_abbr, ccase.case_status, ccase.case_substatus, ccase.case_subsubstatus, ccase.submittedOn, ccase.supervising_attorney, ccase.attorney, ccase.worker, ccase.interpreter_needed, ccase.case_language `case_language`,  
			app.person_id applicant_id, app.person_uuid applicant_uuid,
			IF (app.first_name IS NULL, '', app.first_name) first_name , IF(app.last_name IS NULL, '', app.last_name) last_name, app.full_name, 
			IFNULL(app.language, '') language,
			IF (DATE_FORMAT(app.dob, '%m/%d/%Y') IS NULL, '', DATE_FORMAT(app.dob, '%m/%d/%Y')) dob, app.dob app_dob,
			IF (app.ssn = 'XXXXXXXXX', '', app.ssn) ssn,
			IFNULL(employer.`corporation_id`,-1) employer_id, employer.`corporation_uuid` employer_uuid, employer.`company_name` employer, 
			...
			att.user_id attorney_id, user.user_id, IFNULL(att.nickname, '') as attorney_name, IFNULL(att.user_name, '') as attorney_full_name, IFNULL(user.nickname,'') as worker_name, IFNULL(user.user_name,'') as worker_full_name,
			IFNULL(lien.lien_id, -1) lien_id, IFNULL(settlement.settlement_id, -1) settlement_id
			FROM cse_case ccase
			INNER JOIN (
				SELECT case_id 
                FROM cse_case 
			...
			LEFT OUTER JOIN `cse_user` superatt
			...
			WHERE 1";			
			$sql .= " ORDER by IF (TRIM(app.first_name) = '', TRIM(app.full_name), app.first_name), app.last_name, ccase.case_id, inj.injury_number";
...
$sql = "SELECT DISTINCT 
		inj.injury_id id, ccase.case_id, inj.injury_number, ccase.case_uuid uuid, ccase.case_number, inj.injury_number, inj.adj_number, ccase.rating, 
			IF (DATE_FORMAT(ccase.case_date, '%m/%d/%Y') IS NULL, '', DATE_FORMAT(ccase.case_date, '%m/%d/%Y')) case_date , 
			ccase.case_type, ccase.medical, ccase.td, ccase.rehab,  ccase.edd, ccase.claims,
			venue.venue_uuid, IF(venue.venue IS NULL, '', venue.venue) venue, venue_abbr, ccase.case_status, ccase.case_substatus, ccase.case_subsubstatus, ccase.submittedOn, ccase.attorney, ccase.worker, ccase.interpreter_needed, ccase.case_language `case_language`,  
			app.person_id applicant_id, app.person_uuid applicant_uuid,
...
<html lang="en">
  ...
  <body>
    ...
    <div id="chat_holder">
        <div id="chat_box">chat goes here</div>
        <div id="chat_bottom">chat goes here</div>
    </div>
   ...
   <?php 
	...
	basket
    .require({ url: 'lib/jquery.min.1.10.2.js' })
    .then(function () {
        basket.require(
			{ url: 'lib/underscore-min.js' },
			{ url: 'lib/backbone.js' }
		);
    });
	</script>
