...
$arrUploads = array();
$arrNames = array();
for ($int=$start;$int<($uploads+1);$int++) {
	...
	if ($my_uploader->upload($upload_file_browser, $acceptable_file_types, $default_extension)) {
		...
		$my_uploader->file['name'] = str_replace("." . $extension, "_dor_" . $case_id . "." . $extension, $my_uploader->file['name']);
...
set_time_limit(60);
//die("done");
header ("location:upload_lien.php?case_id=" . $case_id . "&injury_id=" . $injury_id . "&jetfile_id=" . $jetfile_id);
?>