...
for ($int=1;$int<($uploads+1);$int++) {
	...
	$new_stored_file_name = passed_var("file_name_selected_" . $int);
	...
	if ($my_uploader->upload($upload_file_browser, $acceptable_file_types, $default_extension)) {
		...
		*/
		$arrFile = explode(".", $my_uploader->file['name']); //print_r($arrFile);
		...
		//die("else");
		echo "could not upload ".$new_stored_file_name . "<br />";//die;
		$arrKaseNames[] = $upload_file_name;
		//$arrKaseDocs[] = $stored_file_name;
		//echo "101".$path;
		$upload_kase_doc = $path.$new_stored_file_name;
		$my_uploader->save_file($upload_kase_doc, $mode);
		$arrKaseDocs[] = $new_stored_file_name;
		echo "104".$upload_kase_doc;//die;
