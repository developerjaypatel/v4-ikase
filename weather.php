...
if ($ip_record->city=="" || $ip_record->city=="Not Available") {
	...
	//$api_0 = "http://www.infosniper.net/index.php?ip_address=" . $ip;	//pay
	$api_0 = "http://www.ipfingerprints.com/scripts/getIPInfo.php";
	$fields = array("ip"=>$ip);
	//die($api_0);
	$city = post_curl($api_0);
	die($city);
	$doc = new DOMDocument();
	@$html = $doc->loadHTML($city);
	// die(print_r($doc));
	...
	*/
	...
	try {
		...
		//it might be an update to an old version
		$sql = "UPDATE `ikase`.`cse_ip_location` 
		SET city = '" . addslashes($city) . "',
